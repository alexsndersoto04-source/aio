...
`Pool::new(url, maximum)` provides bounded reuse, condition-variable acquisition timeout, RAII return, stats and controlled close. Connections open outside the lock; failed opens release capacity; idle connections close immediately during shutdown and checked-out connections close when returned. VM pool handles and migrations are the next MySQL blocks.
