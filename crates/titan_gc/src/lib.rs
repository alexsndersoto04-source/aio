...
impl GarbageCollector {
    ...
    pub fn with_threshold(bytes: usize) -> Self {
        Self {
            gc_threshold: bytes.max(1),
            ..Self::new()
        }
    }

    pub fn allocate(&mut self, size: usize) -> GcRef {
        if self.allocated_bytes.saturating_add(size) >= self.gc_threshold {
            self.collect();
        }
        let id = self.next_id;
        self.next_id += 1;
    ...
    pub fn set_references(&mut self, object: GcRef, references: Vec<GcRef>) -> bool {
        if references.iter().any(|r| !self.objects.contains_key(r)) {
            return false;
        }
        if let Some(value) = self.objects.get_mut(&object) {
    ...
    pub fn add_root(&mut self, reference: GcRef) -> bool {
        self.objects.contains_key(&reference) && self.roots.insert(reference)
    }
    pub fn remove_root(&mut self, reference: GcRef) -> bool {
    ...
    pub fn collect(&mut self) -> usize {
        ...
        while let Some(reference) = pending.pop() {
            ...
            if let Some(object) = self.objects.get(&reference) {
        ...
        self.roots
            .retain(|reference| self.objects.contains_key(reference));
        self.allocated_bytes = self.objects.values().map(|object| object.size).sum();
...
mod tests {
    use super::*;
    #[test]
    fn traces_transitive_references() {
        let mut gc = GarbageCollector::new();
        let root = gc.allocate(8);
        let child = gc.allocate(8);
        let dead = gc.allocate(8);
        assert!(gc.set_references(root, vec![child]));
        assert!(gc.add_root(root));
