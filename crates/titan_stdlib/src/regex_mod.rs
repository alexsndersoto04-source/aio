...
fn compile(pattern: &str) -> Result<Regex, RegexError> {
    Regex::new(pattern).map_err(|source| RegexError::Compile {
...
mod tests {
    ...
    fn replaces_and_splits() {
        ...
        assert_eq!(
            replace_all(r"(\w+)-(\w+)", "foo-bar baz-qux", "$2-$1").unwrap(),
            "bar-foo qux-baz"
        );
        assert_eq!(
            split(r"\s+", "hola   mundo bonito").unwrap(),
            vec!["hola", "mundo", "bonito"]
        );
    }
    ...
    fn unicode_aware() {
        assert_eq!(
            find_all(r"\p{Letter}+", "hola, mundo 123 café").unwrap(),
            vec!["hola", "mundo", "café"]
        );
    }
}
