...
pub fn year(ts: i64)         -> i64 { to_dt(ts).year() as i64 }
pub fn month(ts: i64)        -> i64 { to_dt(ts).month() as i64 }
pub fn day(ts: i64)          -> i64 { to_dt(ts).day() as i64 }
pub fn hour(ts: i64)         -> i64 { to_dt(ts).hour() as i64 }
pub fn minute(ts: i64)       -> i64 { to_dt(ts).minute() as i64 }
pub fn second(ts: i64)       -> i64 { to_dt(ts).second() as i64 }
pub fn day_of_week(ts: i64)  -> i64 { to_dt(ts).weekday().num_days_from_monday() as i64 }
pub fn day_of_year(ts: i64)  -> i64 { to_dt(ts).ordinal() as i64 }
pub fn week_of_year(ts: i64) -> i64 { to_dt(ts).iso_week().week() as i64 }
...
pub fn days_in_month(y: i64, m: i64) -> i64 {
    ...
    let next_month = if m == 12 { NaiveDate::from_ymd_opt(y as i32 + 1, 1, 1) }
                     else       { NaiveDate::from_ymd_opt(y as i32, m as u32 + 1, 1) };
...
pub fn add_seconds(ts: i64, n: i64) -> i64 { ts + n }
pub fn add_minutes(ts: i64, n: i64) -> i64 { ts + n * 60 }
pub fn add_hours(ts: i64, n: i64)   -> i64 { ts + n * 3600 }
pub fn add_days(ts: i64, n: i64)    -> i64 { ts + n * 86400 }
pub fn add_weeks(ts: i64, n: i64)   -> i64 { ts + n * 604800 }
...
pub fn add_months(ts: i64, n: i64) -> i64 {
    ...
    match NaiveDate::from_ymd_opt(y, m as u32, clamped)
...
pub fn add_years(ts: i64, n: i64) -> i64 { add_months(ts, n * 12) }

pub fn diff_seconds(a: i64, b: i64) -> i64 { a - b }
pub fn diff_minutes(a: i64, b: i64) -> i64 { (a - b) / 60 }
pub fn diff_hours(a: i64, b: i64)   -> i64 { (a - b) / 3600 }
pub fn diff_days(a: i64, b: i64)    -> i64 { (a - b) / 86400 }

// ---------------- Comparaciones ----------------

pub fn is_before(a: i64, b: i64) -> bool { a < b }
pub fn is_after(a: i64, b: i64)  -> bool { a > b }
...
pub fn to_timezone(ts: i64, tz_name: &str) -> Result<String, String> {
    let tz: Tz = tz_name.parse().map_err(|e: chrono_tz::ParseError| e.to_string())?;
...
pub fn common_timezones() -> Vec<String> {
    vec![
        "UTC",
        "America/Caracas", "America/Argentina/Buenos_Aires", "America/Mexico_City",
...
pub fn to_iso(ts: i64) -> String { to_dt(ts).to_rfc3339() }
...
pub fn humanize(ts: i64, now: i64) -> String {
    ...
    let ago = delta < 0;
    let abs = delta.abs();
    let (n, unit) = if abs < 60         { (abs,             "segundo") }
                    else if abs < 3600  { (abs / 60,        "minuto") }
                    else if abs < 86400 { (abs / 3600,      "hora") }
                    else if abs < 604800 { (abs / 86400,    "dia") }
                    else if abs < 2592000 { (abs / 604800,  "semana") }
                    else if abs < 31536000 { (abs / 2592000,"mes") }
                    else                   { (abs / 31536000, "año") };
    let plural = if n == 1 { "" } else { "s" };
    if abs < 5 { return "ahora".into(); }
    if ago { format!("hace {n} {unit}{plural}") }
    else   { format!("en {n} {unit}{plural}") }
}
