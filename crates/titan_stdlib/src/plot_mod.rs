...
fn err<E: std::fmt::Display>(e: E) -> PlotError {
...
pub fn line_svg(
    ...
    if xs.len() != ys.len() {
        return Err(PlotError::Input("xs and ys must have the same length"));
    }
    if xs.is_empty() {
        return Err(PlotError::Input("need at least one point"));
    }
    ...
    chart
        .draw_series(LineSeries::new(series.clone(), BLUE.stroke_width(2)))
        .map_err(err)?;
...
pub fn multi_line_svg(
    ...
    if series.is_empty() {
        return Err(PlotError::Input("need at least one series"));
    }
    for (_, xs, ys) in series {
        if xs.len() != ys.len() {
            return Err(PlotError::Input("each series xs/ys must match"));
        }
        if xs.is_empty() {
            return Err(PlotError::Input("series cannot be empty"));
        }
    }
...
pub fn bar_svg(
    ...
    if labels.is_empty() {
        return Err(PlotError::Input("need at least one bar"));
    }
...
pub fn scatter_svg(
    ...
    chart
        .draw_series(
            xs.iter()
                .zip(ys.iter())
                .map(|(x, y)| Circle::new((*x, *y), 4, dot.filled())),
        )
        .map_err(err)?;
...
pub fn histogram_svg(
    ...
    if values.is_empty() {
        return Err(PlotError::Input("need at least one value"));
    }
    let bins = bins.max(1);
