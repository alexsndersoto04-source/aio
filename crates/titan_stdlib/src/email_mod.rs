...
fn to_build(error: impl std::fmt::Display) -> EmailError { EmailError::Build(error.to_string()) }
fn to_transport(error: impl std::fmt::Display) -> EmailError { EmailError::Transport(error.to_string()) }

fn make_transport(host: &str, port: u16, user: &str, pass: &str) -> Result<SmtpTransport, EmailError> {
    ...
    let builder = match port {
        ...
        587 => SmtpTransport::starttls_relay(host).map_err(to_transport)?.port(587),
        25 => SmtpTransport::builder_dangerous(host).port(25),
        other => SmtpTransport::starttls_relay(host).map_err(to_transport)?.port(other),
    };
...
pub fn send_simple(
    host: &str, port: u16, user: &str, pass: &str,
    ...
    Ok(format!("code={} messages={:?}", response.code(), response.message().collect::<Vec<_>>()))
}

/// Send an HTML + plain-text alternative email.
pub fn send_html(
    host: &str, port: u16, user: &str, pass: &str,
    from: &str, to: &str, subject: &str, text: &str, html: &str,
) -> Result<String, EmailError> {
    let email = Message::builder()
        .from(from.parse().map_err(to_build)?)
        .to(to.parse().map_err(to_build)?)
...
pub fn send_with_attachment(
    host: &str, port: u16, user: &str, pass: &str,
    from: &str, to: &str, subject: &str,
    html: &str, filename: &str, mime: &str, bytes: &[u8],
) -> Result<String, EmailError> {
    let content_type: ContentType = mime.parse().map_err(to_build)?;
    let attachment = Attachment::new(filename.to_string())
        .body(Body::new(bytes.to_vec()), content_type);
...
mod tests {
    ...
    fn live_send_when_configured() {
        let host = std::env::var("TITAN_SMTP_HOST"); let port = std::env::var("TITAN_SMTP_PORT");
        if host.is_err() || port.is_err() { return; }
        ...
        let out = send_simple(&host, port, &user, &pass, &from, &to,
            "TITAN test", "Hola, este email viene del test de titan_stdlib.").unwrap();
