...
pub fn from_codepoints(values: &[u32]) -> Option<String> {
    values.iter().map(|v| char::from_u32(*v)).collect()
}
...
pub fn truncate(text: &str, max_chars: usize, suffix: &str) -> String {
    ...
    let mut result: String = text.chars().take(max_chars - suffix_len).collect();
    result.push_str(suffix);
    result
}
pub fn pad_start(text: &str, width: usize, fill: char) -> String {
    let missing = width.saturating_sub(length(text));
...
mod tests {
    ...
    fn unicode_operations() {
        assert_eq!(length("año"), 3);
        assert_eq!(reverse("año"), "oña");
        assert_eq!(truncate("abcdef", 4, "…"), "abc…");
    }
    #[test]
    fn escaping_and_distance() {
