...
pub enum HttpError {
    ...
    InvalidLength,
    #[error("Transfer-Encoding is not accepted with this codec")]
    TransferEncoding,
    #[error("HTTP/1.1 requires exactly one Host header")]
    Host,
    #[error("invalid response status {0}")]
    InvalidStatus(u16),
    #[error("invalid response header name or value")]
    InvalidResponseHeader,
    #[error("invalid route pattern")]
    RoutePattern,
    #[error("invalid percent encoding in route/query")]
    RouteEncoding,
    #[error("query pair count exceeds configured limit")]
    TooManyQueryPairs,
}
...
pub fn parse_request(
    ...
    let Some(header_end) = buffer.windows(4).position(|window| window == b"\r\n\r\n") else {
        if buffer.len() > limits.headers_bytes {
            return Err(HttpError::HeadersTooLarge);
        }
        return Ok(None);
    };
    if header_end + 4 > limits.headers_bytes {
        return Err(HttpError::HeadersTooLarge);
    }
    ...
    let method = parts
        ...
        .ok_or(HttpError::RequestLine)?;
    let target = parts
        .next()
        .filter(|value| !value.is_empty() && !value.bytes().any(|byte| byte.is_ascii_control()))
        .ok_or(HttpError::RequestLine)?;
    let version = parts.next().ok_or(HttpError::RequestLine)?;
    if parts.next().is_some() {
        return Err(HttpError::RequestLine);
    }
    if version != "HTTP/1.1" && version != "HTTP/1.0" {
        return Err(HttpError::Version(version.into()));
    }
    ...
    for line in lines {
        ...
        if name == "content-length" {
            lengths.push(
                value
                    .parse::<usize>()
                    .map_err(|_| HttpError::InvalidLength)?,
            );
        }
    ...
    if version == "HTTP/1.1" && host_count != 1 {
        return Err(HttpError::Host);
    }
    let body_length = lengths.first().copied().unwrap_or(0);
    if body_length > limits.body_bytes {
        return Err(HttpError::BodyTooLarge);
    }
    let consumed = header_end + 4 + body_length;
    if buffer.len() < consumed {
        return Ok(None);
    }
    let (path, query) = target
        .split_once('?')
        .map(|(path, query)| (path, Some(query.into())))
    ...
    Ok(Some((
        Request {
            ...
            headers,
            body: buffer[header_end + 4..consumed].to_vec(),
            keep_alive,
        },
        consumed,
    )))
...
pub fn build_response(
    ...
    let reason = reason_phrase(status).ok_or(HttpError::InvalidStatus(status))?;
...
pub fn match_route(
    pattern: &str,
    path: &str,
) -> Result<Option<BTreeMap<String, String>>, HttpError> {
    ...
    for (index, part) in pattern_parts.iter().enumerate() {
        if let Some(name) = part.strip_prefix('*') {
            ...
            let rest = path_parts[path_index..].join("/");
            params.insert(
                name.into(),
                crate::encoding::percent_decode(&rest).map_err(|_| HttpError::RouteEncoding)?,
            );
            path_index = path_parts.len();
            break;
        }
        let Some(value) = path_parts.get(path_index) else {
            return Ok(None);
        };
        if let Some(name) = part.strip_prefix(':') {
            ...
            params.insert(
                name.into(),
                crate::encoding::percent_decode(value).map_err(|_| HttpError::RouteEncoding)?,
            );
        } else if *part != *value {
            return Ok(None);
        }
...
pub fn parse_query(
    ...
    for (index, pair) in query.split('&').enumerate() {
        if index >= max_pairs {
            return Err(HttpError::TooManyQueryPairs);
        }
        let (key, value) = pair.split_once('=').unwrap_or((pair, ""));
        let key = crate::encoding::percent_decode(&key.replace('+', " "))
            .map_err(|_| HttpError::RouteEncoding)?;
        let value = crate::encoding::percent_decode(&value.replace('+', " "))
            .map_err(|_| HttpError::RouteEncoding)?;
...
mod tests {
    use super::*;
    #[test]
    fn matches_routes_and_multivalue_queries() {
        let params = match_route("/users/:id/files/*path", "/users/42/files/docs/readme.txt")
            .unwrap()
            .unwrap();
        assert_eq!(params["id"], "42");
        assert_eq!(params["path"], "docs/readme.txt");
        let query = parse_query("tag=rust&tag=titan&q=hello+world", 10).unwrap();
        assert_eq!(query["tag"], vec!["rust", "titan"]);
        assert_eq!(query["q"], vec!["hello world"]);
        assert!(match_route("/users/:id", "/posts/1").unwrap().is_none());
    }
    #[test]
    fn rejects_bad_route_patterns_and_query_limits() {
        assert_eq!(
            match_route("/files/*path/more", "/files/a/more"),
            Err(HttpError::RoutePattern)
        );
        assert_eq!(parse_query("a=1&b=2", 1), Err(HttpError::TooManyQueryPairs));
    }
    #[test]
    fn parses_incremental_keep_alive_request() {
        let request =
            b"POST /users?id=7 HTTP/1.1\r\nHost: localhost\r\nContent-Length: 4\r\n\r\ndataNEXT";
        let (parsed, used) = parse_request(request, &HttpLimits::default())
            .unwrap()
            .unwrap();
        assert_eq!(parsed.path, "/users");
        assert_eq!(parsed.query.as_deref(), Some("id=7"));
        assert_eq!(parsed.body, b"data");
        assert!(parsed.keep_alive);
        assert_eq!(&request[used..], b"NEXT");
    }
    #[test]
    fn rejects_smuggling_and_injection() {
        assert_eq!(
            parse_request(
                b"POST / HTTP/1.1\r\nContent-Length: 1\r\nContent-Length: 2\r\n\r\na",
                &HttpLimits::default()
            ),
            Err(HttpError::ConflictingLength)
        );
        let mut headers = BTreeMap::new();
        headers.insert("X-Test".into(), "ok\r\nInjected: yes".into());
        assert!(build_response(200, &headers, b"", false).is_err());
    }
    #[test]
