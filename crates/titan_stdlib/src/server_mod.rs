...
fn registry() -> &'static Mutex<Registry> {
    ...
    REG.get_or_init(|| Mutex::new(Registry {
        servers: HashMap::new(), requests: HashMap::new(), next_id: 1,
    }))
}
...
pub fn url(handle: i64)  -> Result<String, ServerError> { with_request(handle, |r| r.url().to_string()) }
pub fn path(handle: i64) -> Result<String, ServerError> {
    with_request(handle, |r| r.url().split('?').next().unwrap_or("/").to_string())
}
pub fn query(handle: i64) -> Result<String, ServerError> {
    with_request(handle, |r| {
        let u = r.url();
        u.find('?').map(|i| u[i + 1..].to_string()).unwrap_or_default()
    })
}
pub fn remote_addr(handle: i64) -> Result<String, ServerError> {
    with_request(handle, |r| r.remote_addr().map(|a| a.to_string()).unwrap_or_default())
}

pub fn header(handle: i64, name: &str) -> Result<Option<String>, ServerError> {
    with_request(handle, |r| {
        let target = HeaderField::from_bytes(name.as_bytes()).ok()?;
        r.headers().iter().find(|h| h.field == target).map(|h| h.value.as_str().to_string())
    })
}
...
pub fn respond(handle: i64, status: u16, body: &str) -> Result<(), ServerError> {
    ...
    if let Some(h) = make_header("Content-Type", "text/plain; charset=utf-8") { resp.add_header(h); }
    req.respond(resp)?;
    Ok(())
}
...
pub fn respond_html(handle: i64, status: u16, html: &str) -> Result<(), ServerError> {
    ...
    if let Some(h) = make_header("Content-Type", "text/html; charset=utf-8") { resp.add_header(h); }
    req.respond(resp)?;
    Ok(())
}
...
pub fn respond_json(handle: i64, status: u16, body: &str) -> Result<(), ServerError> {
    let req = take_request(handle)?;
    let mut resp = Response::from_string(body).with_status_code(status);
    if let Some(h) = make_header("Content-Type", "application/json; charset=utf-8") { resp.add_header(h); }
    req.respond(resp)?;
    Ok(())
}

pub fn respond_bytes(handle: i64, status: u16, content_type: &str, data: Vec<u8>) -> Result<(), ServerError> {
    let req = take_request(handle)?;
    let mut resp = Response::from_data(data).with_status_code(status);
    if let Some(h) = make_header("Content-Type", content_type) { resp.add_header(h); }
    req.respond(resp)?;
    Ok(())
}
...
pub fn respond_full(
    handle: i64, status: u16, content_type: &str,
    extra_headers: &[(String, String)], data: Vec<u8>,
) -> Result<(), ServerError> {
    let req = take_request(handle)?;
    let mut resp = Response::from_data(data).with_status_code(status);
    if let Some(h) = make_header("Content-Type", content_type) { resp.add_header(h); }
    for (k, v) in extra_headers {
        ...
        if let Some(h) = make_header(k, v) { resp.add_header(h); }
    }
    req.respond(resp)?;
    Ok(())
}
...
pub fn upgrade_websocket(handle: i64, max_message: usize) -> Result<i64, ServerError> {
    ...
    let accept = ws_codec::accept_key(&key).map_err(|_| ServerError::Upgrade("invalid Sec-WebSocket-Key"))?;
    ...
    let resp = Response::empty(101)
        ...
        .with_header(Header::from_bytes(b"Sec-WebSocket-Accept".as_slice(), accept.as_bytes()).unwrap());

    let stream = req.upgrade("websocket", resp);
    let ws = WsConn { stream, decoder: MessageDecoder::new(max_message) };
    Ok(insert_ws(ws))
}
            ...
            None => {
                ...
                if n == 0 {
                    return Err(ServerError::Io(std::io::Error::new(
                        std::io::ErrorKind::UnexpectedEof, "websocket peer closed",
                    )));
                }
...
mod tests {
    ...
    fn unknown_handles_report_typed_errors() {
        assert!(matches!(method(999_999), Err(ServerError::UnknownRequest(_))));
        assert!(matches!(local_addr(999_999), Err(ServerError::UnknownServer(_))));
        assert!(matches!(ws_recv(999_999), Err(ServerError::UnknownWebSocket(_))));
    }
}
