...
fn validate_opcode(fin:bool,opcode:u8,payload:&[u8])->Result<(),WebSocketError>{match opcode{0|1|2=>{}8..=10=>{if !fin||payload.len()>125{return Err(WebSocketError::ControlFrame)}if opcode==8&&payload.len()==1{return Err(WebSocketError::ControlFrame)}}_=>return Err(WebSocketError::Opcode(opcode))}if opcode==1&&std::str::from_utf8(payload).is_err(){return Err(WebSocketError::InvalidUtf8)}Ok(())}
