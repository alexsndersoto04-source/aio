...
pub fn hide_cursor() -> Result<(), TermError> { execute!(io::stdout(), cursor::Hide)?; Ok(()) }
pub fn show_cursor() -> Result<(), TermError> { execute!(io::stdout(), cursor::Show)?; Ok(()) }

/// Returns `(columns, rows)` for the current terminal.
pub fn size() -> Result<(u16, u16), TermError> { Ok(terminal::size()?) }

pub fn flush() -> Result<(), TermError> { io::stdout().flush()?; Ok(()) }
...
pub fn enable_raw() -> Result<(), TermError> { terminal::enable_raw_mode()?; Ok(()) }
pub fn disable_raw() -> Result<(), TermError> { terminal::disable_raw_mode()?; Ok(()) }
...
pub fn read_key(timeout_ms: u64) -> Result<String, TermError> {
    if !event::poll(Duration::from_millis(timeout_ms))? { return Ok(String::new()); }
    match event::read()? {
        Event::Key(KeyEvent { code, modifiers, .. }) => Ok(format_key(code, modifiers)),
        _ => Ok(String::new()),
    }
}
...
mod tests {
    ...
    fn parses_named_colors() {
        ...
        assert_eq!(parse_color("rgb:10,20,30").unwrap(), Color::Rgb { r: 10, g: 20, b: 30 });
        assert_eq!(parse_color("#ff8800").unwrap(),      Color::Rgb { r: 0xff, g: 0x88, b: 0 });
    ...
    fn parses_attributes() {
        ...
        assert!(matches!(parse_attr("Underline").unwrap(), Attribute::Underlined));
    ...
    fn format_key_examples() {
        ...
        assert_eq!(format_key(KeyCode::Char('c'), KeyModifiers::CONTROL), "Ctrl+c");
