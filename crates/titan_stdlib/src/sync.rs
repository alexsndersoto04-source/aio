...
impl ThreadPool {
    pub fn new(size: usize, queue_capacity: usize) -> Result<Self, PoolError> {
        ...
        for index in 0..size {
            ...
            workers.push(
                thread::Builder::new()
                    .name(format!("titan-worker-{index}"))
                    .spawn(move || loop {
...
impl<T> Shared<T> {
    ...
    pub fn map<R>(&self, function: impl FnOnce(&mut T) -> R) -> R {
        function(&mut self.lock())
    }
}
pub fn channel<T>() -> (mpsc::Sender<T>, mpsc::Receiver<T>) {
    mpsc::channel()
}
pub fn bounded_channel<T>(capacity: usize) -> (mpsc::SyncSender<T>, mpsc::Receiver<T>) {
...
mod tests {
    ...
    fn pool_runs_jobs_and_joins() {
        let value = Shared::new(0);
        {
            let pool = ThreadPool::new(2, 4).unwrap();
            for _ in 0..4 {
                let value = value.clone();
                pool.execute(move || value.map(|v| *v += 1)).unwrap();
            }
        }
        assert_eq!(*value.lock(), 4);
    }
}
