...
pub fn unix_seconds() -> Result<u64, std::time::SystemTimeError> {
    Ok(SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs())
}
pub fn unix_millis() -> Result<u128, std::time::SystemTimeError> {
...
mod tests {
    use super::*;
    #[test]
    fn validates_seconds() {
        assert!(seconds(1.5).is_some());
        assert!(seconds(-1.0).is_none());
        assert!(seconds(f64::NAN).is_none());
    }
}
