...
where
    F: FnOnce(&Tokenizer) -> Result<R, TokenizeError>,
{
...
pub fn load(path: &str) -> Result<i64, TokenizeError> {
    let tokenizer =
        Tokenizer::from_file(path).map_err(|e| TokenizeError::Backend(e.to_string()))?;
    Ok(insert(tokenizer))
}
...
pub fn from_json(json: &str) -> Result<i64, TokenizeError> {
    let tokenizer: Tokenizer = json
        .parse()
        .map_err(|e: tokenizers::Error| TokenizeError::Backend(e.to_string()))?;
    Ok(insert(tokenizer))
}
...
pub fn close(handle: i64) {
    if let Ok(mut reg) = registry().lock() {
        reg.tokenizers.remove(&handle_key(handle));
    }
}
    ...
    with(handle, |t| {
        ...
        if ids.len() > max_length {
            ...
            if add_special_tokens {
                ...
                if let Some(v) = last_tok {
                    tokens[max_length - 1] = v;
                }
                if let Some(v) = last_type {
                    types[max_length - 1] = v;
                }
                if let Some(v) = last_mask {
                    mask[max_length - 1] = v;
                }
                if let Some(v) = last_spec {
                    special[max_length - 1] = v;
                }
            }
        } else {
...
pub fn decode(
    ...
    with(handle, |t| {
        t.decode(ids, skip_special_tokens)
            .map_err(|e| TokenizeError::Backend(e.to_string()))
    })
}
