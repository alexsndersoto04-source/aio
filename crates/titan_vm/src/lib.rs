...
pub fn val_to_string(value: &Value) -> String {
    match value {
        ...
        Value::Enum {
            ...
            Some(value) => format!("{}::{}({})", name, variant, val_to_string(value)),
            None => format!("{}::{}", name, variant),
        },
        Value::Closure { function, .. } => format!("<closure:{function}>"),
        Value::Task(id) => format!("<task:{id}>"),
        Value::ChannelSender(id) => format!("<sender:{id}>"),
        Value::ChannelReceiver(id) => format!("<receiver:{id}>"),
        Value::TcpListener(id) => format!("<tcp-listener:{id}>"),
        Value::TcpStream(id) => format!("<tcp-stream:{id}>"),
        Value::HttpRouter(id) => format!("<http-router:{id}>"),
        Value::TlsStream(id) => format!("<tls-stream:{id}>"),
        Value::TlsServerConfig(id) => format!("<tls-server-config:{id}>"),
        Value::WebSocketDecoder(id) => format!("<websocket-decoder:{id}>"),
        Value::WebSocket(id) => format!("<websocket:{id}>"),
        Value::ServerControl(id) => format!("<server-control:{id}>"),
        Value::Sqlite(id) => format!("<sqlite:{id}>"),
        Value::SqlitePool(id) => format!("<sqlite-pool:{id}>"),
        Value::Postgres(id) => format!("<postgres:{id}>"),
        Value::PostgresPool(id) => format!("<postgres-pool:{id}>"),
        Value::Mysql(id) => format!("<mysql:{id}>"),
        Value::MysqlPool(id) => format!("<mysql-pool:{id}>"),
    }
}
...
struct HttpRouterState {
    ...
    middleware: Vec<HttpCallable>,
    after: Vec<HttpCallable>,
    error_handler: Option<HttpCallable>,
}
#[derive(Clone)]
enum WebSocketTransport {
    Tcp(Arc<Mutex<TcpStream>>),
    Tls(Arc<Mutex<titan_tls::TlsStream>>),
}
struct WebSocketConnection {
    transport: WebSocketTransport,
    decoder: Mutex<titan_stdlib::websocket::MessageDecoder>,
    require_mask: bool,
    mask_outgoing: bool,
...
enum DatabaseHandle {
    Direct(titan_sqlite::Database),
    Pooled(titan_sqlite::PooledConnection),
}
enum PostgresHandle {
    Direct(titan_postgres::Database),
    Pooled(titan_postgres::PooledConnection),
}
enum MysqlHandle {
    Direct(titan_mysql::Database),
    Pooled(titan_mysql::PooledConnection),
}
impl std::ops::Deref for MysqlHandle {
    type Target = titan_mysql::Database;
    fn deref(&self) -> &Self::Target {
        match self {
            Self::Direct(database) => database,
            Self::Pooled(database) => database,
        }
    }
}
impl std::ops::DerefMut for MysqlHandle {
...
impl std::ops::Deref for PostgresHandle {
    type Target = titan_postgres::Database;
    fn deref(&self) -> &Self::Target {
        match self {
            Self::Direct(database) => database,
            Self::Pooled(database) => database,
...
impl std::ops::DerefMut for PostgresHandle {
    fn deref_mut(&mut self) -> &mut Self::Target {
        match self {
            Self::Direct(database) => database,
            Self::Pooled(database) => database,
...
impl std::ops::Deref for DatabaseHandle {
    type Target = titan_sqlite::Database;
    fn deref(&self) -> &Self::Target {
        match self {
            Self::Direct(database) => database,
            Self::Pooled(database) => database,
...
impl std::ops::DerefMut for DatabaseHandle {
    fn deref_mut(&mut self) -> &mut Self::Target {
        match self {
            Self::Direct(database) => database,
            Self::Pooled(database) => database,
...
impl RuntimeState {
    fn new() -> Self {
        Self {
            ...
            websocket_decoders: Mutex::new(HashMap::new()),
            websockets: Mutex::new(HashMap::new()),
            server_controls: Mutex::new(HashMap::new()),
...
impl Vm {
    ...
    pub fn with_output_sender(mut self, output: std::sync::mpsc::Sender<String>) -> Self {
    ...
    fn run_internal(
        ...
        if let Some(hook) = debugger.as_deref_mut() {
            hook.terminated(result.as_ref().err());
        }
        result.map(Some)
    }
    ...
    fn execute(
        ...
        if captures.len() != function.captures {
            return Err(VmError::Type(format!(
                "closure expected {} captures, found {}",
                function.captures,
                captures.len()
            )));
        }
        let mut locals = vec![Value::Nil; function.locals.max(args.len() + captures.len())];
        for (slot, value) in locals.iter_mut().zip(captures.into_iter().chain(args)) {
            *slot = value;
        }
        ...
        while ip < function.code.len() {
            ...
            match function.code[ip].clone() {
                ...
                Op::PushChar(v) => stack.push(Value::Char(v)),
                Op::PushNil => stack.push(Value::Nil),
                Op::PushStr(index) => stack.push(Value::Str(
                    self.module
                        .string_table
                        .get(index)
                        .cloned()
                ...
                Op::PushLocal(index) => stack.push(locals.get(index).cloned().ok_or_else(
                    || VmError::InvalidLocal {
                ...
                Op::Dup => {
                    let value = stack
                        .last()
                        .cloned()
                        .ok_or_else(|| VmError::StackUnderflow(function.name.clone()))?;
                    stack.push(value);
                }
                Op::Add => {
                ...
                Op::Neg => {
                    ...
                    stack.push(match value {
                        Value::Int(v) => Value::Int(v.checked_neg().ok_or(VmError::Overflow)?),
                ...
                Op::BitNot => {
                    ...
                    if let Value::Int(v) = value {
                        ...
                        return Err(VmError::Type("bitwise not requires int".into()));
                    }
                }
                Op::Jump(target) => {
                    ip = target;
                ...
                Op::JumpIfFalse(target) => {
                    let condition = pop(&mut stack, &function.name)?;
                    if !truthy(&condition) {
                        ip = target;
                        continue;
                    }
                    ...
                    let captured = captures
                        ...
                        .map(|slot| {
                            locals
                                .get(slot)
                                .cloned()
                                .ok_or_else(|| VmError::InvalidLocal {
                                    function: function.name.clone(),
                                    index: slot,
                                })
                        })
                ...
                Op::CallValue(argc) => {
                    ...
                    } = callable
                    {
                        ...
                        return Err(VmError::Type(
                            "attempted to call a non-function value".into(),
                        ));
                    }
                }
                Op::TryCall(argc) => {
                ...
                Op::SpawnQuota => {
                    ...
                    let Value::Closure {
                        ...
                        return Err(VmError::Type(
                            "std::runtime::spawn_quota requires a closure".into(),
                        ));
                    };
                    let quota_bytes = positive_limit(
                        pop(&mut stack, &function.name)?,
                ...
                Op::RuntimeMemoryLimit => {
                    let val = if self.memory_limit == usize::MAX {
                        -1i64
                    } else {
                        self.memory_limit as i64
                    };
                ...
                Op::RuntimeGcSetThreshold => {
                    let bytes = positive_limit(
                        pop(&mut stack, &function.name)?,
                        "std::runtime::gc_set_threshold",
                    )?;
                ...
                Op::RuntimeHeapDump => {
                    ...
                    let Value::Str(path) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("heap_dump path must be string".into()));
                    };
                ...
                Op::RuntimeBenchmark => {
                    ...
                    let Value::Closure {
                        ...
                        return Err(VmError::Type(
                            "std::runtime::benchmark requires a closure".into(),
                        ));
                    };
                    let iters = positive_limit(
                        pop(&mut stack, &function.name)?,
                        "std::runtime::benchmark iterations",
                    )?;
                ...
                Op::JoinTask => {
                    let Value::Task(task_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("join requires a task".into()));
                    };
                    let record = self
                        .runtime
                        .tasks
                        ...
                        .map_err(|_| VmError::Type("task registry poisoned".into()))?
                        .remove(&task_id)
                        .ok_or(VmError::UnknownTask(task_id))?;
                    let result = record
                        .result
                        .recv()
                        .map_err(|_| VmError::TaskPanicked(task_id))?;
                    record
                        .handle
                        .join()
                        .map_err(|_| VmError::TaskPanicked(task_id))?;
                ...
                Op::JoinTaskTimeout => {
                    ...
                    let Value::Task(task_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("join_timeout requires a task".into()));
                    };
                    let record = self
                        .runtime
                        .tasks
                        ...
                        .remove(&task_id)
                        .ok_or(VmError::UnknownTask(task_id))?;
                    match record.result.recv_timeout(timeout) {
                        Ok(result) => {
                            record
                                .handle
                                .join()
                        ...
                        Err(mpsc::RecvTimeoutError::Disconnected) => {
                            return Err(VmError::TaskPanicked(task_id))
                        }
                    }
                }
                Op::CancelTask => {
                    let Value::Task(task_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("cancel requires a task".into()));
                    };
                    let cancelled = self
                        .runtime
                        .tasks
                ...
                Op::ChannelSend => {
                    let value = pop(&mut stack, &function.name)?;
                    let Value::ChannelSender(channel_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("send requires a channel sender".into()));
                    };
                    let channel = self
                        .runtime
                        .channels
                    ...
                    channel
                        .sender
                        .send(value)
                        .map_err(|_| VmError::ChannelDisconnected(channel_id))?;
                    stack.push(Value::Nil);
                }
                Op::ChannelRecv => {
                    let Value::ChannelReceiver(channel_id) = pop(&mut stack, &function.name)?
                    else {
                        return Err(VmError::Type("recv requires a channel receiver".into()));
                    };
                    let channel = self
                        .runtime
                        .channels
                ...
                Op::ChannelRecvTimeout => {
                    ...
                    let Value::ChannelReceiver(channel_id) = pop(&mut stack, &function.name)?
                        ...
                        return Err(VmError::Type("recv_timeout requires a receiver".into()));
                    };
                    let channel = self
                        .runtime
                        .channels
                    ...
                    match result {
                        Ok(value) => stack.push(option_some(value)),
                        Err(mpsc::RecvTimeoutError::Timeout) => stack.push(option_none()),
                        Err(mpsc::RecvTimeoutError::Disconnected) => {
                ...
                Op::ChannelSelect => {
                    ...
                    let ids: Vec<u64> = receivers
                        ...
                        .map(|value| match value {
                            ...
                            _ => Err(VmError::Type(
                                "select requires an array of receivers".into(),
                            )),
                        })
                        .collect::<Result<_, _>>()?;
                    ...
                    'select: loop {
                        for (index, channel_id) in ids.iter().enumerate() {
                            let channel = self
                                ...
                                .ok_or(VmError::UnknownChannel(*channel_id))?;
                            let received = {
                                let receiver = channel.receiver.lock().map_err(|_| {
                                    VmError::Type("channel receiver poisoned".into())
                                })?;
                                receiver.try_recv()
                            };
                            match received {
                ...
                Op::TcpLocalAddr => {
                    ...
                    let Value::TcpListener(listener_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("tcp_local_addr requires a listener".into()));
                    };
                    let listener = self
                        .runtime
                        .listeners
                    ...
                    let address = listener
                        .local_addr()
                        .map_err(|error| network_error("std::net::tcp_local_addr", error))?;
                    stack.push(Value::Str(address.to_string()));
                }
                Op::TcpAccept => {
                    require_network(self.capabilities, "std::net::tcp_accept")?;
                    let Value::TcpListener(listener_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("tcp_accept requires a listener".into()));
                    };
                    let listener = self
                        .runtime
                        .listeners
                ...
                Op::TcpRead => {
                    ...
                    let Value::Int(maximum) = maximum else {
                        return Err(VmError::Type("tcp_read maximum must be int".into()));
                    };
                    let maximum = usize::try_from(maximum)
                        .map_err(|_| VmError::Type("tcp_read maximum must be nonnegative".into()))?
                        .min(16 * 1024 * 1024);
                    let Value::TcpStream(stream_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("tcp_read requires a stream".into()));
                    };
                    let stream = socket_stream(&self.runtime, stream_id)?;
                    let mut buffer = vec![0; maximum];
                    let count = stream
                        .lock()
                        .map_err(|_| VmError::Type("TCP stream poisoned".into()))?
                        .read(&mut buffer)
                        .map_err(|error| network_error("std::net::tcp_read", error))?;
                    buffer.truncate(count);
                    stack.push(Value::Bytes(buffer));
                }
                Op::TcpWrite => {
                    require_network(self.capabilities, "std::net::tcp_write")?;
                    let data = pop(&mut stack, &function.name)?;
                    let data = match data {
                        Value::Bytes(data) => data,
                        Value::Str(data) => data.into_bytes(),
                        _ => {
                            return Err(VmError::Type("tcp_write requires bytes or string".into()))
                        }
                    };
                    let Value::TcpStream(stream_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("tcp_write requires a stream".into()));
                    };
                    let stream = socket_stream(&self.runtime, stream_id)?;
                    stream
                        .lock()
                        .map_err(|_| VmError::Type("TCP stream poisoned".into()))?
                        .write_all(&data)
                        .map_err(|error| network_error("std::net::tcp_write", error))?;
                    stack.push(Value::Int(data.len() as i64));
                }
                Op::TcpSetTimeout => {
                    require_network(self.capabilities, "std::net::tcp_set_timeout")?;
                    let timeout = timeout_value(pop(&mut stack, &function.name)?)?;
                    let Value::TcpStream(stream_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("tcp_set_timeout requires a stream".into()));
                    };
                    let stream = socket_stream(&self.runtime, stream_id)?;
                    let stream = stream
                        .lock()
                    ...
                    stream
                        ...
                        .map_err(|error| network_error("std::net::tcp_set_timeout", error))?;
                    stack.push(Value::Nil);
                }
                Op::TcpClose => {
                    require_network(self.capabilities, "std::net::tcp_close")?;
                    let removed = match pop(&mut stack, &function.name)? {
                        Value::TcpStream(id) => self
                            .runtime
                            .streams
                            .lock()
                            .map_err(|_| VmError::Type("stream registry poisoned".into()))?
                        ...
                        _ => {
                            return Err(VmError::Type(
                                "tcp_close requires a stream or listener".into(),
                            ))
                        }
                    };
                    if removed {
                ...
                Op::HttpServeConnection => {
                    ...
                    let maximum = pop(&mut stack, &function.name)?;
                    let Value::Int(maximum) = maximum else {
                        return Err(VmError::Type("maximum requests must be int".into()));
                    };
                    let maximum = usize::try_from(maximum)
                        .map_err(|_| VmError::Type("maximum requests must be positive".into()))?;
                    if maximum == 0 || maximum > 10_000 {
                        return Err(VmError::Type(
                            "maximum requests must be between 1 and 10000".into(),
                        ));
                    }
                    let handler = pop(&mut stack, &function.name)?;
                    let Value::Closure {
                        function: handler_function,
                        captures: handler_captures,
                    } = handler
                    else {
                        return Err(VmError::Type("HTTP handler must be a closure".into()));
                    };
                    let Value::TcpListener(listener_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("serve_connection requires a listener".into()));
                    };
                    let listener = self
                        .runtime
                        .listeners
                    ...
                    let (mut stream, peer) = listener
                        .accept()
                        .map_err(|error| network_error("std::http::serve_connection", error))?;
                    let mut buffer = Vec::new();
                    'requests: for _ in 0..maximum {
                        let request = loop {
                            if let Some((request, consumed)) = titan_stdlib::http::parse_request(
                                &buffer,
                                &titan_stdlib::http::HttpLimits::default(),
                            )
                            .map_err(|error| VmError::Native {
                                function: "std::http::serve_connection".into(),
                                message: error.to_string(),
                            })? {
                            ...
                            let count = stream.read(&mut chunk).map_err(|error| {
                                network_error("std::http::serve_connection", error)
                            })?;
                            if count == 0 {
                                break 'requests;
                            }
                            buffer.extend_from_slice(&chunk[..count]);
                            if buffer.len() > 16 * 1024 * 1024 + 64 * 1024 {
                                return Err(VmError::Type(
                                    "HTTP connection buffer limit exceeded".into(),
                                ));
                            }
                        };
                        let request_keep_alive = request.keep_alive;
                        let response_value = self.execute(
                            handler_function,
                            vec![http_request_value(request, &peer.to_string())],
                            handler_captures.clone(),
                            depth + 1,
                            debugger,
                        )?;
                        let response_parts =
                            http_response_value(response_value, request_keep_alive)?;
                        let response = titan_stdlib::http::build_response(
                            response_parts.status,
                            &response_parts.headers,
                            &response_parts.body,
                            response_parts.keep_alive,
                        )
                        .map_err(|error| VmError::Native {
                            function: "std::http::serve_connection".into(),
                            message: error.to_string(),
                ...
                Op::HttpRouteAdd => {
                    ...
                    let Value::Closure {
                        ...
                        return Err(VmError::Type("route handler must be closure".into()));
                    };
                    let Value::Str(pattern) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("route pattern must be string".into()));
                    };
                    let Value::Str(method) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("route method must be string".into()));
                    };
                    let Value::HttpRouter(router_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("route requires router".into()));
                    };
                    titan_stdlib::http::validate_route_pattern(&pattern).map_err(|error| {
                        VmError::Native {
                            function: "std::http::route".into(),
                            message: error.to_string(),
                        }
                    })?;
                    let router = http_router(&self.runtime, router_id)?;
                    router
                        .lock()
                        .map_err(|_| VmError::Type("HTTP router poisoned".into()))?
                        .routes
                        .push(HttpRoute {
                ...
                Op::HttpMiddlewareAdd => {
                    let middleware = pop(&mut stack, &function.name)?;
                    let Value::Closure {
                        function: middleware_function,
                        captures,
                    } = middleware
                    else {
                        return Err(VmError::Type("middleware must be closure".into()));
                    };
                    let Value::HttpRouter(router_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("middleware requires router".into()));
                    };
                    let router = http_router(&self.runtime, router_id)?;
                    router
                        .lock()
                ...
                Op::HttpAfterAdd => {
                    ...
                    let Value::Closure {
                        ...
                        return Err(VmError::Type("response middleware must be closure".into()));
                    };
                    let Value::HttpRouter(router_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("after requires router".into()));
                    };
                    let router = http_router(&self.runtime, router_id)?;
                    router
                        .lock()
                ...
                Op::HttpErrorHandlerAdd => {
                    ...
                    let Value::Closure {
                        ...
                        return Err(VmError::Type("error handler must be closure".into()));
                    };
                    let Value::HttpRouter(router_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("on_error requires router".into()));
                    };
                    let router = http_router(&self.runtime, router_id)?;
                    router
                        .lock()
                        .map_err(|_| VmError::Type("HTTP router poisoned".into()))?
                        .error_handler = Some(HttpCallable {
                        function: handler_function,
                        captures,
                    });
                    stack.push(Value::Nil);
                ...
                Op::HttpDispatch => {
                    ...
                    let mut request = pop(&mut stack, &function.name)?;
                    let Value::HttpRouter(router_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("dispatch requires router".into()));
                    };
                    let router = http_router(&self.runtime, router_id)?;
                    let (middleware, routes, after, error_handler) = {
                        let router = router
                            .lock()
                            .map_err(|_| VmError::Type("HTTP router poisoned".into()))?;
                        (
                            router.middleware.clone(),
                            router.routes.clone(),
                            router.after.clone(),
                            router.error_handler.clone(),
                        )
                    };
                    for layer in middleware {
                        ...
                        if !matches!(&request, Value::Map(_)) {
                            return Err(VmError::Type("middleware must return request map".into()));
                        }
                    }
                    let (method, path) = request_method_path(&request)?;
                    let mut matched = None;
                    for route in routes {
                        if route.method == method {
                            if let Some(params) =
                                titan_stdlib::http::match_route(&route.pattern, &path).map_err(
                                    |error| VmError::Native {
                                        function: "std::http::dispatch".into(),
                                        message: error.to_string(),
                                    },
                                )?
                            {
                    ...
                    let handler_result = if let Some((route, params)) = matched {
                        if let Value::Map(map) = &mut request {
                            map.insert(
                                "params".into(),
                                Value::Map(
                                    params
                    ...
                    let mut response = match handler_result {
                        ...
                        Err(error) => {
                            ...
                            self.execute(
                                handler.function,
                                vec![request_for_error, http_error_value(&error)],
                                handler.captures,
                                depth + 1,
                                debugger,
                            )?
                    ...
                    for layer in after.into_iter().rev() {
                        ...
                        if !matches!(&response, Value::Map(_)) {
                            return Err(VmError::Type(
                                "response middleware must return response map".into(),
                            ));
                        }
                    }
                ...
                Op::TlsConnect => {
                    require_network(self.capabilities, "std::tls::connect")?;
                    let Value::Str(server_name) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("TLS server name must be string".into()));
                    };
                    let Value::Str(address) = pop(&mut stack, &function.name)? else {
                ...
                Op::TlsServerConfig => {
                    require_network(self.capabilities, "std::tls::server_config")?;
                    if !self.capabilities.filesystem {
                        return Err(VmError::PermissionDenied {
                    ...
                    let Value::Str(key) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("TLS key path must be string".into()));
                    };
                ...
                Op::TlsAccept => {
                    require_network(self.capabilities, "std::tls::accept")?;
                    let Value::TlsServerConfig(config_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("tls_accept requires server config".into()));
                    };
                    let Value::TcpListener(listener_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("tls_accept requires listener".into()));
                    };
                    let config = self
                        .runtime
                        .tls_configs
                        ...
                        .get(&config_id)
                        .cloned()
                        .ok_or_else(|| VmError::Type("unknown TLS config".into()))?;
                    let listener = self
                        .runtime
                        .listeners
                        .lock()
                    ...
                    let stream =
                        titan_tls::accept(socket, config).map_err(|error| VmError::Native {
                ...
                Op::TlsRead => {
                    require_network(self.capabilities, "std::tls::read")?;
                    let maximum = pop(&mut stack, &function.name)?;
                    let Value::Int(maximum) = maximum else {
                        return Err(VmError::Type("TLS read maximum must be int".into()));
                    };
                    let maximum = usize::try_from(maximum)
                        .map_err(|_| VmError::Type("TLS read maximum must be nonnegative".into()))?
                        .min(16 * 1024 * 1024);
                    let Value::TlsStream(stream_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("tls_read requires stream".into()));
                    };
                    let stream = tls_stream(&self.runtime, stream_id)?;
                    let mut buffer = vec![0; maximum];
                    let count = stream
                        .lock()
                        .map_err(|_| VmError::Type("TLS stream poisoned".into()))?
                        .read(&mut buffer)
                        .map_err(|error| network_error("std::tls::read", error))?;
                    buffer.truncate(count);
                    stack.push(Value::Bytes(buffer));
                }
                Op::TlsWrite => {
                    require_network(self.capabilities, "std::tls::write")?;
                    let data = pop(&mut stack, &function.name)?;
                    let data = match data {
                        Value::Bytes(data) => data,
                        Value::Str(data) => data.into_bytes(),
                        _ => {
                            return Err(VmError::Type("tls_write requires bytes or string".into()))
                        }
                    };
                    let Value::TlsStream(stream_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("tls_write requires stream".into()));
                    };
                    let stream = tls_stream(&self.runtime, stream_id)?;
                    stream
                        .lock()
                        .map_err(|_| VmError::Type("TLS stream poisoned".into()))?
                        .write_all(&data)
                        .map_err(|error| network_error("std::tls::write", error))?;
                    stack.push(Value::Int(data.len() as i64));
                }
                Op::TlsClose => {
                    require_network(self.capabilities, "std::tls::close")?;
                ...
                Op::WsDecoderNew => {
                    ...
                    self.runtime
                        .websocket_decoders
                        .lock()
                        .map_err(|_| VmError::Type("WebSocket decoder registry poisoned".into()))?
                        .insert(
                            id,
                            Arc::new(Mutex::new(titan_stdlib::websocket::MessageDecoder::new(
                                maximum,
                            ))),
                        );
                    permit.commit();
                ...
                Op::WsDecoderPush => {
                    ...
                    let data = match data {
                        Value::Bytes(data) => data,
                        _ => return Err(VmError::Type("decoder_push requires bytes".into())),
                    };
                    let Value::WebSocketDecoder(id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("decoder_push requires decoder".into()));
                    };
                    let decoder = websocket_decoder(&self.runtime, id)?;
                    decoder
                        .lock()
                        .map_err(|_| VmError::Type("WebSocket decoder poisoned".into()))?
                        .push(&data)
                        .map_err(|error| VmError::Native {
                            function: "std::ws::decoder_push".into(),
                            message: error.to_string(),
                        })?;
                    stack.push(Value::Nil);
                }
                Op::WsDecoderNext => {
                    let require = pop(&mut stack, &function.name)?;
                    let Value::Bool(require) = require else {
                        return Err(VmError::Type(
                            "decoder_next mask policy must be bool".into(),
                        ));
                    };
                    let Value::WebSocketDecoder(id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("decoder_next requires decoder".into()));
                    };
                    let decoder = websocket_decoder(&self.runtime, id)?;
                    let message = decoder
                        .lock()
                        ...
                        .next(Some(require))
                        .map_err(|error| VmError::Native {
                            function: "std::ws::decoder_next".into(),
                            message: error.to_string(),
                        })?;
                    stack.push(
                        message
                            .map(websocket_message_value)
                            .map(option_some)
                ...
                Op::WsConnect => {
                    ...
                    let maximum =
                        positive_limit(pop(&mut stack, &function.name)?, "WebSocket maximum")?;
                    let Value::Str(protocol) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("WebSocket protocol must be string".into()));
                    };
                    let Value::Str(url) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("WebSocket URL must be string".into()));
                    };
                ...
                Op::WsAttachTcp => {
                    let maximum =
                        positive_limit(pop(&mut stack, &function.name)?, "WebSocket maximum")?;
                    let Value::Bool(server_side) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("WebSocket server_side must be bool".into()));
                    };
                    let Value::TcpStream(stream_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("attach_tcp requires TCP stream".into()));
                    };
                    let transport =
                ...
                Op::WsAttachTls => {
                    ...
                    let Value::TlsStream(stream_id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("attach_tls requires TLS stream".into()));
                    };
                    let transport =
                ...
                Op::WsSendText => {
                    let Value::Str(text) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("send_text requires string".into()));
                    };
                    let socket = pop_websocket(&self.runtime, pop(&mut stack, &function.name)?)?;
                    websocket_send(&socket, 1, text.as_bytes())?;
                    stack.push(Value::Nil);
                }
                Op::WsSendBinary => {
                    let Value::Bytes(data) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("send_binary requires bytes".into()));
                    };
                    let socket = pop_websocket(&self.runtime, pop(&mut stack, &function.name)?)?;
                    websocket_send(&socket, 2, &data)?;
                    stack.push(Value::Nil);
                }
                Op::WsReceive => {
                    let socket_value = pop(&mut stack, &function.name)?;
                    let Value::WebSocket(socket_id) = socket_value else {
                        return Err(VmError::Type("receive requires WebSocket".into()));
                    };
                    let socket = pop_websocket(&self.runtime, Value::WebSocket(socket_id))?;
                    let message = websocket_receive(&socket)?;
                ...
                Op::WsClose => {
                    let Value::Str(reason) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("close reason must be string".into()));
                    };
                    let code = pop(&mut stack, &function.name)?;
                    let Value::Int(code) = code else {
                        return Err(VmError::Type("close code must be int".into()));
                    };
                    let code = u16::try_from(code)
                        .map_err(|_| VmError::Type("close code out of range".into()))?;
                    let socket_value = pop(&mut stack, &function.name)?;
                    let Value::WebSocket(socket_id) = socket_value else {
                        return Err(VmError::Type("close requires WebSocket".into()));
                    };
                    let socket = pop_websocket(&self.runtime, Value::WebSocket(socket_id))?;
                ...
                Op::ServerControlNew => {
                    ...
                    self.runtime
                        .server_controls
                        .lock()
                        .map_err(|_| VmError::Type("server control registry poisoned".into()))?
                        .insert(
                            id,
                ...
                Op::ServerTryAcquire => {
                    ...
                    let acquired = server_try_acquire(&control);
                ...
                Op::ServerShutdown => {
                    let control = server_control(&self.runtime, pop(&mut stack, &function.name)?)?;
                    control.shutting_down.store(true, Ordering::Release);
                    stack.push(Value::Nil);
                }
                Op::ServerStats => {
                    let control = server_control(&self.runtime, pop(&mut stack, &function.name)?)?;
                    stack.push(server_stats(&control));
                }
                Op::ServerHealthResponse => {
                    let control = server_control(&self.runtime, pop(&mut stack, &function.name)?)?;
                    stack.push(server_health_response(&control));
                }
                Op::SqliteOpen => {
                    if !self.capabilities.filesystem {
                        return Err(VmError::PermissionDenied {
                            function: "std::sqlite::open".into(),
                            capability: "Filesystem".into(),
                        });
                    }
                    let Value::Str(path) = pop(&mut stack, &function.name)? else {
                ...
                operation @ (Op::SqliteExecute | Op::SqliteQuery) => {
                    let params = sqlite_params(pop(&mut stack, &function.name)?)?;
                    let Value::Str(sql) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("SQL must be string".into()));
                    };
                    let database =
                        sqlite_database(&self.runtime, pop(&mut stack, &function.name)?)?;
                    let mut database = database
                        .lock()
                        .map_err(|_| VmError::Type("SQLite connection poisoned".into()))?;
                    if matches!(operation, Op::SqliteExecute) {
                        stack.push(Value::Int(
                            database.execute(&sql, &params).map_err(sqlite_error)? as i64,
                        ));
                    } else {
                        let rows = database.query(&sql, &params).map_err(sqlite_error)?;
                        stack.push(Value::Array(rows.into_iter().map(sqlite_row).collect()));
                    }
                }
                operation @ (Op::SqliteBegin | Op::SqliteCommit | Op::SqliteRollback) => {
                    let database =
                        sqlite_database(&self.runtime, pop(&mut stack, &function.name)?)?;
                    let mut database = database
                        .lock()
                        .map_err(|_| VmError::Type("SQLite connection poisoned".into()))?;
                    match operation {
                        Op::SqliteBegin => database.begin(),
                        Op::SqliteCommit => database.commit(),
                        _ => database.rollback(),
                    }
                    .map_err(sqlite_error)?;
                    stack.push(Value::Nil);
                ...
                Op::SqliteMigrate => {
                    let migrations = sqlite_migrations(pop(&mut stack, &function.name)?)?;
                    let database =
                        sqlite_database(&self.runtime, pop(&mut stack, &function.name)?)?;
                    let count = database
                        .lock()
                ...
                Op::SqlitePoolStats => {
                    ...
                    stack.push(Value::Map(BTreeMap::from([
                        ...
                        ("checked_out".into(), Value::Int(stats.checked_out as i64)),
                        ("closed".into(), Value::Bool(stats.closed)),
                    ])));
                }
                Op::SqlitePoolHealth => {
                    let timeout = timeout_value(pop(&mut stack, &function.name)?)?;
                    let pool = sqlite_pool(&self.runtime, pop(&mut stack, &function.name)?)?;
                    stack.push(Value::Bool(pool.health_check(timeout)));
                }
                Op::SqlitePoolClose => {
                    let Value::SqlitePool(id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("pool_close requires SQLite pool".into()));
                    };
                    let pool = self
                        .runtime
                        .sqlite_pools
                ...
                operation @ (Op::PostgresConnect | Op::PostgresConnectTls) => {
                    require_network(self.capabilities, "std::postgres::connect")?;
                    let Value::Str(url) = pop(&mut stack, &function.name)? else {
                ...
                operation @ (Op::PostgresExecute | Op::PostgresQuery) => {
                    let params = postgres_params(pop(&mut stack, &function.name)?)?;
                    let Value::Str(sql) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("SQL must be string".into()));
                    };
                    let database =
                        postgres_database(&self.runtime, pop(&mut stack, &function.name)?)?;
                    let mut database = database
                        .lock()
                        .map_err(|_| VmError::Type("PostgreSQL connection poisoned".into()))?;
                    if matches!(operation, Op::PostgresExecute) {
                        stack.push(Value::Int(
                            i64::try_from(database.execute(&sql, &params).map_err(postgres_error)?)
                                .unwrap_or(i64::MAX),
                        ));
                    } else {
                ...
                operation @ (Op::PostgresBegin | Op::PostgresCommit | Op::PostgresRollback) => {
                    let database =
                        postgres_database(&self.runtime, pop(&mut stack, &function.name)?)?;
                    let mut database = database
                        .lock()
                        .map_err(|_| VmError::Type("PostgreSQL connection poisoned".into()))?;
                    match operation {
                        Op::PostgresBegin => database.begin(),
                        Op::PostgresCommit => database.commit(),
                        _ => database.rollback(),
                    }
                    .map_err(postgres_error)?;
                    stack.push(Value::Nil);
                ...
                Op::PostgresMigrate => {
                    let migrations = postgres_migrations(pop(&mut stack, &function.name)?)?;
                    let database =
                        postgres_database(&self.runtime, pop(&mut stack, &function.name)?)?;
                    let count = database
                        .lock()
                ...
                Op::PostgresPoolNew => {
                    require_network(self.capabilities, "std::postgres::pool")?;
                ...
                Op::PostgresPoolStats => {
                    ...
                    stack.push(Value::Map(BTreeMap::from([
                        ...
                        ("checked_out".into(), Value::Int(stats.checked_out as i64)),
                        ("closed".into(), Value::Bool(stats.closed)),
                        ("tls".into(), Value::Bool(stats.tls)),
                    ])));
                }
                Op::PostgresPoolHealth => {
                    let timeout = timeout_value(pop(&mut stack, &function.name)?)?;
                    let pool = postgres_pool(&self.runtime, pop(&mut stack, &function.name)?)?;
                    stack.push(Value::Bool(pool.health_check(timeout)));
                }
                Op::PostgresPoolClose => {
                    let Value::PostgresPool(id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("pool_close requires PostgreSQL pool".into()));
                    };
                    let pool = self
                        .runtime
                        .postgres_pools
                ...
                Op::MysqlConnect => {
                    require_network(self.capabilities, "std::mysql::connect")?;
                    let Value::Str(url) = pop(&mut stack, &function.name)? else {
                ...
                operation @ (Op::MysqlExecute | Op::MysqlQuery) => {
                    let params = mysql_params(pop(&mut stack, &function.name)?)?;
                    let Value::Str(sql) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("SQL must be string".into()));
                    };
                    ...
                    let mut database = database
                        .lock()
                        .map_err(|_| VmError::Type("MySQL connection poisoned".into()))?;
                    if matches!(operation, Op::MysqlExecute) {
                        stack.push(Value::Int(
                            i64::try_from(database.execute(&sql, &params).map_err(mysql_error)?)
                                .unwrap_or(i64::MAX),
                        ));
                    } else {
                        stack.push(Value::Array(
                ...
                operation @ (Op::MysqlBegin | Op::MysqlCommit | Op::MysqlRollback) => {
                    let database = mysql_database(&self.runtime, pop(&mut stack, &function.name)?)?;
                    let mut database = database
                        .lock()
                        .map_err(|_| VmError::Type("MySQL connection poisoned".into()))?;
                    match operation {
                        Op::MysqlBegin => database.begin(),
                        Op::MysqlCommit => database.commit(),
                        _ => database.rollback(),
                    }
                    .map_err(mysql_error)?;
                    stack.push(Value::Nil);
                ...
                Op::MysqlMigrate => {
                    let migrations = mysql_migrations(pop(&mut stack, &function.name)?)?;
                    let database = mysql_database(&self.runtime, pop(&mut stack, &function.name)?)?;
                    let count = database
                        .lock()
                        .map_err(|_| VmError::Type("MySQL connection poisoned".into()))?
                ...
                Op::MysqlLastId => {
                    ...
                    let id = database
                        ...
                        .map_err(|_| VmError::Type("MySQL connection poisoned".into()))?
                        .last_insert_id();
                    stack.push(Value::Int(i64::try_from(id).unwrap_or(i64::MAX)));
                }
                Op::MysqlClose => {
                ...
                Op::MysqlPoolClose => {
                    let Value::MysqlPool(id) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("pool_close requires MySQL pool".into()));
                    };
                    let pool = self
                        .runtime
                        .mysql_pools
                ...
                operation @ (Op::DbExecute | Op::DbQuery) => {
                    let params = pop(&mut stack, &function.name)?;
                    let Value::Str(sql) = pop(&mut stack, &function.name)? else {
                        return Err(VmError::Type("SQL must be string".into()));
                    };
                    let database = pop(&mut stack, &function.name)?;
                    let query = matches!(operation, Op::DbQuery);
                    match database {
                        Value::Sqlite(id) => {
                            let db = sqlite_database(&self.runtime, Value::Sqlite(id))?;
                            let mut db = db
                                .lock()
                                .map_err(|_| VmError::Type("SQLite connection poisoned".into()))?;
                            ...
                            if query {
                                ...
                                stack.push(Value::Int(
                                    db.execute(&sql, &params).map_err(sqlite_error)? as i64,
                                ))
                            }
                        }
                        Value::Postgres(id) => {
                            ...
                            let mut db = db.lock().map_err(|_| {
                                VmError::Type("PostgreSQL connection poisoned".into())
                            })?;
                            ...
                            if query {
                                ...
                                stack.push(Value::Int(
                                    i64::try_from(
                                        db.execute(&sql, &params).map_err(postgres_error)?,
                                    )
                                    .unwrap_or(i64::MAX),
                                ))
                            }
                        ...
                        Value::Mysql(id) => {
                            let db = mysql_database(&self.runtime, Value::Mysql(id))?;
                            let mut db = db
                                .lock()
                                .map_err(|_| VmError::Type("MySQL connection poisoned".into()))?;
                            let params = mysql_params(params)?;
                            if query {
                                stack.push(Value::Array(
                                    db.query(&sql, &params)
                                        .map_err(mysql_error)?
                                ...
                                stack.push(Value::Int(
                                    i64::try_from(db.execute(&sql, &params).map_err(mysql_error)?)
                                        .unwrap_or(i64::MAX),
                                ))
                            }
                        }
                        _ => {
                            return Err(VmError::Type(
                                "std::db operation requires database connection".into(),
                            ))
                        }
                    }
                }
                operation @ (Op::DbBegin | Op::DbCommit | Op::DbRollback) => {
                    let database = pop(&mut stack, &function.name)?;
                    match database {
                        Value::Sqlite(id) => {
                            let db = sqlite_database(&self.runtime, Value::Sqlite(id))?;
                        ...
                        _ => {
                            return Err(VmError::Type(
                                "std::db transaction requires database connection".into(),
                            ))
                        }
                    }
                    stack.push(Value::Nil);
                ...
                Op::DbMigrate => {
                    let migrations = pop(&mut stack, &function.name)?;
                    let database = pop(&mut stack, &function.name)?;
                    let count = match database {
                        ...
                        Value::Postgres(id) => {
                            let db = postgres_database(&self.runtime, Value::Postgres(id))?;
                            let migrations = postgres_migrations(migrations)?;
                            let result = db
                                .lock()
                                .map_err(|_| {
                                    VmError::Type("PostgreSQL connection poisoned".into())
                        ...
                        Value::Mysql(id) => {
                            let db = mysql_database(&self.runtime, Value::Mysql(id))?;
                            let migrations = mysql_migrations(migrations)?;
                            let result = db
                                .lock()
                                .map_err(|_| VmError::Type("MySQL connection poisoned".into()))?
                                .migrate(&migrations)
                        ...
                        _ => {
                            return Err(VmError::Type(
                                "std::db migrate requires database connection".into(),
                            ))
                        }
                    };
                    stack.push(Value::Int(count as i64));
                ...
                Op::DbClose => {
                    let database = pop(&mut stack, &function.name)?;
                    let closed = match database {
                        Value::Sqlite(id) => self
                            .runtime
                            .sqlite
                            .lock()
                            .map_err(|_| VmError::Type("SQLite registry poisoned".into()))?
                        ...
                        _ => {
                            return Err(VmError::Type(
                                "std::db close requires database connection".into(),
                            ))
                        }
                    };
                    if closed {
                ...
                Op::DbPing => {
                    let database = pop(&mut stack, &function.name)?;
                    let healthy = match database {
                        Value::Sqlite(id) => sqlite_database(&self.runtime, Value::Sqlite(id))?
                            .lock()
                            .map_err(|_| VmError::Type("SQLite connection poisoned".into()))?
                            .ping()
                            .unwrap_or(false),
                        ...
                        Value::Mysql(id) => mysql_database(&self.runtime, Value::Mysql(id))?
                            .lock()
                            .map_err(|_| VmError::Type("MySQL connection poisoned".into()))?
                            .ping()
                            .unwrap_or(false),
                        _ => {
                            return Err(VmError::Type(
                                "std::db ping requires database connection".into(),
                            ))
                        }
                    };
                    stack.push(Value::Bool(healthy));
                }
                Op::Try => {
                    let value = pop(&mut stack, &function.name)?;
                    match value {
                        ...
                        _ => {
                            return Err(VmError::Type(
                                "operator ? requires Result or Option".into(),
                            ))
                        }
                    }
                }
                Op::CallNative { name, argc } => {
                    ...
                    // Otherwise programs could bypass the sandbox memory limit
                    // via natives such as std::array::concat or std::text::*.
                ...
                Op::ToString => {
                    let value = pop(&mut stack, &function.name)?;
                    self.track_allocation(64)?;
                    stack.push(Value::Str(val_to_string(&value)));
                }
                Op::NewArray(count) => {
                ...
                Op::Index => {
                    ...
                    let target = pop(&mut stack, &function.name)?;
                    let value = match (target, index_value) {
                        (Value::Map(values), Value::Str(key)) => values
                            .get(&key)
                            .cloned()
                            .ok_or(VmError::UnknownField(key))?,
                        (target, Value::Int(index)) => {
                            ...
                            match target {
                                Value::Array(v) | Value::Tuple(v) => {
                                    v.get(index).cloned().ok_or(VmError::IndexOutOfBounds {
                                        index,
                                        length: v.len(),
                                    })?
                                }
                                Value::Str(v) => v.chars().nth(index).map(Value::Char).ok_or(
                                ...
                                _ => {
                                    return Err(VmError::Type(
                                        "value is not indexable by integer".into(),
                                    ))
                                }
                            }
                        }
                        _ => {
                            return Err(VmError::Type(
                                "index must be int, or string for maps".into(),
                            ))
                        }
                    };
                    stack.push(value);
                ...
                Op::GetField(field) => {
                    let value = pop(&mut stack, &function.name)?;
                    match value {
                        Value::Struct { fields, .. } | Value::Map(fields) => stack.push(
                            fields
                                .get(&field)
                                .cloned()
                                .ok_or(VmError::UnknownField(field))?,
                        ),
                        _ => {
                            return Err(VmError::Type(
                                "field access requires a struct or map".into(),
                            ))
                        }
                    }
                }
                Op::NewEnum {
                    ...
                    let payload = if has_payload {
                        Some(Box::new(pop(&mut stack, &function.name)?))
                    } else {
                        None
                    };
                    stack.push(Value::Enum {
                ...
                Op::EnumPayload => {
                    ...
                    } = value
                    {
                        ...
                        return Err(VmError::Type("enum variant has no payload".into()));
                    }
                }
                Op::Nop => {}
...
fn http_request_value(request: titan_stdlib::http::Request, peer: &str) -> Value {
    let mut map = BTreeMap::new();
...
fn http_router(runtime: &RuntimeState, id: u64) -> Result<Arc<Mutex<HttpRouterState>>, VmError> {
    runtime
        ...
        .get(&id)
        .cloned()
        .ok_or_else(|| VmError::Type(format!("unknown HTTP router {id}")))
}
fn request_method_path(request: &Value) -> Result<(String, String), VmError> {
    let Value::Map(map) = request else {
        return Err(VmError::Type("dispatch request must be map".into()));
    };
    let method = match map.get("method") {
        Some(Value::Str(value)) => value.to_ascii_uppercase(),
        _ => return Err(VmError::Type("request.method must be string".into())),
    };
    let path = match map.get("path") {
        Some(Value::Str(value)) => value.clone(),
        _ => return Err(VmError::Type("request.path must be string".into())),
    };
    Ok((method, path))
}
...
fn http_not_found() -> Value {
    Value::Map(BTreeMap::from([
        ...
        ("body".into(), Value::Str("Not Found".into())),
...
) -> Result<HttpResponseParts, VmError> {
    let Value::Map(mut map) = value else {
        return Err(VmError::Type(
            "HTTP handler must return a response map".into(),
        ));
    };
    let status = match map.remove("status").unwrap_or(Value::Int(200)) {
        Value::Int(status) => u16::try_from(status)
        ...
        Value::Map(headers) => headers
            ...
            .map(|(key, value)| {
                if let Value::Str(value) = value {
                    ...
                    Err(VmError::Type(
                        "HTTP response headers must be strings".into(),
                    ))
                }
            })
            .collect::<Result<_, _>>()?,
        _ => return Err(VmError::Type("HTTP response headers must be a map".into())),
    };
    let body = match map.remove("body").unwrap_or(Value::Bytes(Vec::new())) {
        Value::Bytes(body) => body,
        Value::Str(body) => body.into_bytes(),
        _ => {
            return Err(VmError::Type(
                "HTTP response body must be bytes or string".into(),
            ))
        }
    };
    let keep_alive = match map
        .remove("keep_alive")
        .unwrap_or(Value::Bool(request_keep_alive))
    {
        Value::Bool(value) => value && request_keep_alive,
        _ => {
            return Err(VmError::Type(
                "HTTP response keep_alive must be bool".into(),
            ))
        }
    };
...
fn require_network(capabilities: RuntimeCapabilities, function: &str) -> Result<(), VmError> {
...
fn network_error(function: &str, error: std::io::Error) -> VmError {
    VmError::Native {
        function: function.into(),
        message: error.to_string(),
    }
}
fn mysql_pool(runtime: &RuntimeState, value: Value) -> Result<titan_mysql::Pool, VmError> {
    let Value::MysqlPool(id) = value else {
        return Err(VmError::Type("operation requires MySQL pool".into()));
    };
    runtime
        .mysql_pools
        .lock()
        ...
        .get(&id)
        .cloned()
        .ok_or_else(|| VmError::Type(format!("unknown MySQL pool {id}")))
}
fn mysql_error(error: titan_mysql::MyError) -> VmError {
    VmError::Native {
        function: "std::mysql".into(),
        message: error.to_string(),
    }
}
fn mysql_database(
    ...
    value: Value,
) -> Result<Arc<Mutex<MysqlHandle>>, VmError> {
    let Value::Mysql(id) = value else {
        return Err(VmError::Type("operation requires MySQL connection".into()));
    };
    runtime
        .mysql
        .lock()
        ...
        .get(&id)
        .cloned()
        .ok_or_else(|| VmError::Type(format!("unknown MySQL connection {id}")))
}
fn mysql_migrations(value: Value) -> Result<Vec<titan_mysql::Migration>, VmError> {
    array_value(value)?
        .into_iter()
        .map(|value| {
            let Value::Map(mut map) = value else {
                return Err(VmError::Type("migration must be map".into()));
            };
            let version = match map.remove("version") {
                Some(Value::Int(value)) => value,
                _ => return Err(VmError::Type("migration.version must be int".into())),
            };
            let name = match map.remove("name") {
                Some(Value::Str(value)) => value,
                _ => return Err(VmError::Type("migration.name must be string".into())),
            };
            let sql = match map.remove("sql") {
                Some(Value::Str(value)) => value,
                _ => return Err(VmError::Type("migration.sql must be string".into())),
            };
            Ok(titan_mysql::Migration { version, name, sql })
        })
        .collect()
}
fn mysql_params(value: Value) -> Result<Vec<titan_mysql::DbValue>, VmError> {
    array_value(value)?
        .into_iter()
        .map(|value| match value {
            Value::Nil => Ok(titan_mysql::DbValue::Null),
            Value::Int(value) => Ok(titan_mysql::DbValue::Integer(value)),
            Value::Float(value) if value.is_finite() => Ok(titan_mysql::DbValue::Real(value)),
            Value::Str(value) => Ok(titan_mysql::DbValue::Text(value)),
            Value::Bytes(value) => Ok(titan_mysql::DbValue::Bytes(value)),
            _ => Err(VmError::Type(
                "MySQL parameters must be nil, int, finite float, string, or bytes".into(),
            )),
        })
        .collect()
}
fn mysql_row(row: BTreeMap<String, titan_mysql::DbValue>) -> Value {
    Value::Map(
        row.into_iter()
            .map(|(name, value)| {
                (
                    name,
                    match value {
                        titan_mysql::DbValue::Null => Value::Nil,
                        titan_mysql::DbValue::Integer(value) => Value::Int(value),
                        titan_mysql::DbValue::Unsigned(value) => i64::try_from(value)
                            .map(Value::Int)
                            .unwrap_or_else(|_| Value::Str(value.to_string())),
                        titan_mysql::DbValue::Real(value) => Value::Float(value),
                        titan_mysql::DbValue::Text(value)
                        | titan_mysql::DbValue::Date(value)
                        | titan_mysql::DbValue::Time(value) => Value::Str(value),
                        titan_mysql::DbValue::Bytes(value) => Value::Bytes(value),
                    },
                )
            })
            .collect(),
...
fn postgres_pool(runtime: &RuntimeState, value: Value) -> Result<titan_postgres::Pool, VmError> {
    let Value::PostgresPool(id) = value else {
        return Err(VmError::Type("operation requires PostgreSQL pool".into()));
    };
    runtime
        .postgres_pools
        .lock()
        ...
        .get(&id)
        .cloned()
        .ok_or_else(|| VmError::Type(format!("unknown PostgreSQL pool {id}")))
}
...
fn postgres_database(
    runtime: &RuntimeState,
    value: Value,
) -> Result<Arc<Mutex<PostgresHandle>>, VmError> {
    let Value::Postgres(id) = value else {
        return Err(VmError::Type(
            "operation requires PostgreSQL connection".into(),
        ));
    };
    runtime
        .postgres
        ...
        .get(&id)
        .cloned()
        .ok_or_else(|| VmError::Type(format!("unknown PostgreSQL connection {id}")))
}
fn postgres_migrations(value: Value) -> Result<Vec<titan_postgres::Migration>, VmError> {
    array_value(value)?
        .into_iter()
        .map(|value| {
            let Value::Map(mut map) = value else {
            ...
            Ok(titan_postgres::Migration { version, name, sql })
        })
        .collect()
}
fn postgres_params(value: Value) -> Result<Vec<titan_postgres::DbValue>, VmError> {
    array_value(value)?
        .into_iter()
        .map(|value| match value {
            Value::Nil => Ok(titan_postgres::DbValue::Null),
            Value::Bool(value) => Ok(titan_postgres::DbValue::Bool(value)),
            Value::Int(value) => Ok(titan_postgres::DbValue::Integer(value)),
            Value::Float(value) if value.is_finite() => Ok(titan_postgres::DbValue::Real(value)),
            Value::Str(value) => Ok(titan_postgres::DbValue::Text(value)),
            Value::Bytes(value) => Ok(titan_postgres::DbValue::Bytes(value)),
            value => vm_json(value).map(titan_postgres::DbValue::Json),
        })
        .collect()
}
fn vm_json(value: Value) -> Result<serde_json::Value, VmError> {
    Ok(match value {
        ...
        Value::Float(value) => serde_json::Number::from_f64(value)
            ...
            .ok_or_else(|| VmError::Type("JSON float must be finite".into()))?,
        Value::Str(value) => value.into(),
        Value::Array(values) | Value::Tuple(values) => {
            serde_json::Value::Array(values.into_iter().map(vm_json).collect::<Result<_, _>>()?)
        }
        Value::Map(values) => serde_json::Value::Object(
            values
                .into_iter()
                .map(|(key, value)| Ok((key, vm_json(value)?)))
                .collect::<Result<_, VmError>>()?,
        ),
        _ => return Err(VmError::Type("unsupported PostgreSQL parameter".into())),
    })
}
fn postgres_row(row: BTreeMap<String, titan_postgres::DbValue>) -> Value {
    Value::Map(
        row.into_iter()
            .map(|(name, value)| {
                (
                    name,
                    match value {
                        titan_postgres::DbValue::Null => Value::Nil,
                        titan_postgres::DbValue::Bool(value) => Value::Bool(value),
                        titan_postgres::DbValue::Integer(value) => Value::Int(value),
                        titan_postgres::DbValue::Real(value) => Value::Float(value),
                        titan_postgres::DbValue::Text(value) => Value::Str(value),
                        titan_postgres::DbValue::Bytes(value) => Value::Bytes(value),
                        titan_postgres::DbValue::Json(value) => json_vm(value),
                    },
                )
            })
            .collect(),
...
fn json_vm(value: serde_json::Value) -> Value {
    match value {
        ...
        serde_json::Value::Array(values) => Value::Array(values.into_iter().map(json_vm).collect()),
        serde_json::Value::Object(values) => Value::Map(
            values
                .into_iter()
                .map(|(key, value)| (key, json_vm(value)))
                .collect(),
        ),
    }
}
fn sqlite_pool(runtime: &RuntimeState, value: Value) -> Result<titan_sqlite::Pool, VmError> {
    let Value::SqlitePool(id) = value else {
        return Err(VmError::Type("operation requires SQLite pool".into()));
    };
    runtime
        .sqlite_pools
        .lock()
        ...
        .get(&id)
        .cloned()
        .ok_or_else(|| VmError::Type(format!("unknown SQLite pool {id}")))
}
fn sqlite_error(error: titan_sqlite::DbError) -> VmError {
    VmError::Native {
        function: "std::sqlite".into(),
        message: error.to_string(),
    }
}
fn sqlite_database(
    runtime: &RuntimeState,
    value: Value,
) -> Result<Arc<Mutex<DatabaseHandle>>, VmError> {
    let Value::Sqlite(id) = value else {
        return Err(VmError::Type("operation requires SQLite connection".into()));
    };
    runtime
        .sqlite
        .lock()
        ...
        .get(&id)
        .cloned()
        .ok_or_else(|| VmError::Type(format!("unknown SQLite connection {id}")))
}
fn sqlite_migrations(value: Value) -> Result<Vec<titan_sqlite::Migration>, VmError> {
    array_value(value)?
        .into_iter()
        .map(|value| {
            let Value::Map(mut map) = value else {
            ...
            Ok(titan_sqlite::Migration { version, name, sql })
        })
        .collect()
}
fn sqlite_params(value: Value) -> Result<Vec<titan_sqlite::DbValue>, VmError> {
    let values = array_value(value)?;
    values
        .into_iter()
        .map(|value| match value {
            Value::Nil => Ok(titan_sqlite::DbValue::Null),
            Value::Int(value) => Ok(titan_sqlite::DbValue::Integer(value)),
            Value::Float(value) if value.is_finite() => Ok(titan_sqlite::DbValue::Real(value)),
            Value::Str(value) => Ok(titan_sqlite::DbValue::Text(value)),
            Value::Bytes(value) => Ok(titan_sqlite::DbValue::Blob(value)),
            _ => Err(VmError::Type(
                "SQLite parameters must be nil, int, finite float, string, or bytes".into(),
            )),
        })
        .collect()
}
fn sqlite_row(row: BTreeMap<String, titan_sqlite::DbValue>) -> Value {
    Value::Map(
        row.into_iter()
            .map(|(name, value)| {
                (
                    name,
                    match value {
                        titan_sqlite::DbValue::Null => Value::Nil,
                        titan_sqlite::DbValue::Integer(value) => Value::Int(value),
                        titan_sqlite::DbValue::Real(value) => Value::Float(value),
                        titan_sqlite::DbValue::Text(value) => Value::Str(value),
                        titan_sqlite::DbValue::Blob(value) => Value::Bytes(value),
                    },
                )
            })
            .collect(),
...
fn server_control(runtime: &RuntimeState, value: Value) -> Result<Arc<ServerControl>, VmError> {
    let Value::ServerControl(id) = value else {
        return Err(VmError::Type("operation requires server control".into()));
    };
    runtime
        .server_controls
        .lock()
        ...
        .get(&id)
        .cloned()
        .ok_or_else(|| VmError::Type(format!("unknown server control {id}")))
}
...
fn server_stats(control: &ServerControl) -> Value {
    ...
    Value::Map(BTreeMap::from([
        ...
        (
            "active".into(),
            Value::Int(control.active.load(Ordering::Acquire) as i64),
        ),
        (
            "accepted".into(),
            Value::Int(control.accepted.load(Ordering::Relaxed) as i64),
        ),
        (
            "rejected".into(),
            Value::Int(control.rejected.load(Ordering::Relaxed) as i64),
        ),
        (
            "completed".into(),
            Value::Int(control.completed.load(Ordering::Relaxed) as i64),
        ),
        ("ready".into(), Value::Bool(!shutting_down)),
        ("healthy".into(), Value::Bool(true)),
        ("shutting_down".into(), Value::Bool(shutting_down)),
    ]))
}
fn server_health_response(control: &ServerControl) -> Value {
    let ready = !control.shutting_down.load(Ordering::Acquire);
    let active = control.active.load(Ordering::Acquire);
    let body = format!(
        "{{\"status\":\"{}\",\"ready\":{},\"healthy\":true,\"active\":{}}}",
        if ready { "ok" } else { "draining" },
        ready,
        active
    );
    Value::Map(BTreeMap::from([
        ...
        (
            ...
            Value::Map(BTreeMap::from([
                ...
                ("Cache-Control".into(), Value::Str("no-store".into())),
            ])),
        ),
        ("body".into(), Value::Bytes(body.into_bytes())),
        ("keep_alive".into(), Value::Bool(false)),
    ]))
}
...
fn websocket_connect(
    ...
    let address = if parsed.host.contains(':') {
        format!("[{}]:{}", parsed.host, parsed.port)
    } else {
        format!("{}:{}", parsed.host, parsed.port)
    };
    let transport = if parsed.secure {
        WebSocketTransport::Tls(Arc::new(Mutex::new(
            titan_tls::connect(&address, &parsed.host, titan_tls::client_config()).map_err(
                |error| VmError::Native {
                    function: "std::ws::connect".into(),
                    message: error.to_string(),
                },
        ...
        let socket = TcpStream::connect(&address)
            .map_err(|error| network_error("std::ws::connect", error))?;
        socket
    ...
    let key = titan_stdlib::websocket::client_key().map_err(|error| VmError::Native {
        function: "std::ws::connect".into(),
        message: error.to_string(),
    })?;
    ...
    loop {
        ...
        if response.len() > 64 * 1024 {
            return Err(VmError::Type(
                "WebSocket handshake headers too large".into(),
            ));
        }
        let mut chunk = [0u8; 4096];
        let count = websocket_transport_read(&transport, &mut chunk)?;
        if count == 0 {
            return Err(VmError::WebSocketDisconnected);
        }
        response.extend_from_slice(&chunk[..count]);
    }
    let consumed =
        titan_stdlib::websocket::validate_accept_response(&response, &key).map_err(|error| {
            VmError::Native {
                function: "std::ws::connect".into(),
                message: error.to_string(),
            }
        })?;
    titan_stdlib::websocket::validate_subprotocol_response(&response, protocol).map_err(
        |error| VmError::Native {
            function: "std::ws::connect".into(),
            message: error.to_string(),
        },
...
fn positive_limit(value: Value, name: &str) -> Result<usize, VmError> {
    let Value::Int(value) = value else {
        return Err(VmError::Type(format!("{name} must be int")));
    };
    let value =
        usize::try_from(value).map_err(|_| VmError::Type(format!("{name} must be positive")))?;
    if value == 0 || value > 64 * 1024 * 1024 {
        return Err(VmError::Type(format!(
            "{name} must be between 1 and 64 MiB"
        )));
    }
    Ok(value)
}
    ...
    runtime
        ...
        .insert(
            ...
            Arc::new(WebSocketConnection {
                transport,
                decoder: Mutex::new(titan_stdlib::websocket::MessageDecoder::new(maximum)),
                require_mask: server_side,
                mask_outgoing: !server_side,
...
fn pop_websocket(
    runtime: &RuntimeState,
    value: Value,
) -> Result<Arc<WebSocketConnection>, VmError> {
    let Value::WebSocket(id) = value else {
        return Err(VmError::Type("operation requires WebSocket".into()));
    };
    runtime
        .websockets
        .lock()
        ...
        .get(&id)
        .cloned()
        .ok_or_else(|| VmError::Type(format!("unknown WebSocket {id}")))
}
fn websocket_transport_write(transport: &WebSocketTransport, data: &[u8]) -> Result<(), VmError> {
    match transport {
        WebSocketTransport::Tcp(stream) => stream
            .lock()
            .map_err(|_| VmError::Type("TCP stream poisoned".into()))?
            .write_all(data)
            .map_err(|error| network_error("std::ws", error)),
        WebSocketTransport::Tls(stream) => stream
            .lock()
            .map_err(|_| VmError::Type("TLS stream poisoned".into()))?
            .write_all(data)
            .map_err(|error| network_error("std::ws", error)),
...
fn websocket_write(socket: &WebSocketConnection, data: &[u8]) -> Result<(), VmError> {
    websocket_transport_write(&socket.transport, data)
}
fn websocket_read(socket: &WebSocketConnection, buffer: &mut [u8]) -> Result<usize, VmError> {
    websocket_transport_read(&socket.transport, buffer)
}
fn websocket_send(socket: &WebSocketConnection, opcode: u8, payload: &[u8]) -> Result<(), VmError> {
    if socket.close_sent.load(Ordering::Acquire) {
        return Err(VmError::Type("WebSocket is closing".into()));
    }
    let frame =
        titan_stdlib::websocket::encode_frame_with_policy(opcode, payload, socket.mask_outgoing)
            .map_err(|error| VmError::Native {
                function: "std::ws".into(),
                message: error.to_string(),
            })?;
    websocket_write(socket, &frame)
}
fn websocket_close(socket: &WebSocketConnection, code: u16, reason: &str) -> Result<(), VmError> {
    if !matches!(code, 1000..=1003 | 1007..=1014 | 3000..=4999) || reason.len() > 123 {
        return Err(VmError::Type(
            "invalid WebSocket close code or reason".into(),
        ));
    }
    ...
    let frame =
        titan_stdlib::websocket::encode_frame_with_policy(8, &payload, socket.mask_outgoing)
            .map_err(|error| VmError::Native {
...
) -> Result<titan_stdlib::websocket::Message, VmError> {
    loop {
        if let Some(message) = socket
            .decoder
            .lock()
            .map_err(|_| VmError::Type("WebSocket decoder poisoned".into()))?
            .next(Some(socket.require_mask))
            .map_err(|error| VmError::Native {
                function: "std::ws::receive".into(),
                message: error.to_string(),
            })?
        {
            match message {
                titan_stdlib::websocket::Message::Ping(payload) => {
                ...
                titan_stdlib::websocket::Message::Close { code, reason } => {
                    if !socket.close_sent.load(Ordering::Acquire) {
                        websocket_close(socket, code.unwrap_or(1000), &reason)?
                    }
                    return Ok(titan_stdlib::websocket::Message::Close { code, reason });
                }
                message => return Ok(message),
            }
        }
        let mut bytes = [0u8; 8192];
        let count = websocket_read(socket, &mut bytes)?;
        if count == 0 {
            return Err(VmError::WebSocketDisconnected);
        }
        socket
...
fn websocket_decoder(
    runtime: &RuntimeState,
    id: u64,
) -> Result<Arc<Mutex<titan_stdlib::websocket::MessageDecoder>>, VmError> {
    runtime
        .websocket_decoders
        .lock()
        .map_err(|_| VmError::Type("WebSocket decoder registry poisoned".into()))?
        .get(&id)
        .cloned()
        .ok_or_else(|| VmError::Type(format!("unknown WebSocket decoder {id}")))
}
fn websocket_message_value(message: titan_stdlib::websocket::Message) -> Value {
    ...
    match message {
        ...
        Message::Close { code, reason } => Value::Map(BTreeMap::from([
            ...
            (
                "code".into(),
                code.map(|code| Value::Int(code as i64))
                    .unwrap_or(Value::Nil),
            ),
...
fn take_socket_stream(runtime: &RuntimeState, id: u64) -> Result<Arc<Mutex<TcpStream>>, VmError> {
    runtime
        .streams
        .lock()
        .map_err(|_| VmError::Type("stream registry poisoned".into()))?
        .remove(&id)
        .ok_or_else(|| VmError::Type(format!("unknown TCP stream {id}")))
}
fn take_tls_stream(
    runtime: &RuntimeState,
    id: u64,
) -> Result<Arc<Mutex<titan_tls::TlsStream>>, VmError> {
    runtime
        .tls_streams
        .lock()
        .map_err(|_| VmError::Type("TLS stream registry poisoned".into()))?
        .remove(&id)
        .ok_or_else(|| VmError::Type(format!("unknown TLS stream {id}")))
}
fn tls_stream(
    runtime: &RuntimeState,
    id: u64,
...
fn socket_stream(runtime: &RuntimeState, id: u64) -> Result<Arc<Mutex<TcpStream>>, VmError> {
    runtime
        .streams
        .lock()
        .map_err(|_| VmError::Type("stream registry poisoned".into()))?
...
fn timeout_value(value: Value) -> Result<Duration, VmError> {
    if let Value::Int(milliseconds) = value {
        Ok(Duration::from_millis(
            u64::try_from(milliseconds).map_err(|_| VmError::InvalidTimeout)?,
        ))
    } else {
...
fn pop(stack: &mut Vec<Value>, function: &str) -> Result<Value, VmError> {
    stack
        .pop()
        .ok_or_else(|| VmError::StackUnderflow(function.into()))
}
fn take_args(stack: &mut Vec<Value>, count: usize, function: &str) -> Result<Vec<Value>, VmError> {
    if stack.len() < count {
        return Err(VmError::StackUnderflow(function.into()));
    }
...
fn binary<F>(stack: &mut Vec<Value>, function: &str, operation: F) -> Result<(), VmError>
where
...
fn compare<F>(stack: &mut Vec<Value>, function: &str, operation: F) -> Result<(), VmError>
where
...
{
    binary(stack, function, |a, b| match (a, b) {
        (Value::Int(a), Value::Int(b)) => Ok(Value::Int(operation(a, b))),
        _ => Err(VmError::Type("bitwise operation requires integers".into())),
    })
}
fn add(a: Value, b: Value) -> Result<Value, VmError> {
    match (a, b) {
        (Value::Int(a), Value::Int(b)) => {
            Ok(Value::Int(a.checked_add(b).ok_or(VmError::Overflow)?))
        }
        ...
        _ => Err(VmError::Type(
            "addition requires matching numbers or strings".into(),
        )),
    }
}
fn sub(a: Value, b: Value) -> Result<Value, VmError> {
    match (a, b) {
        (Value::Int(a), Value::Int(b)) => {
            Ok(Value::Int(a.checked_sub(b).ok_or(VmError::Overflow)?))
        }
        (Value::Float(a), Value::Float(b)) => Ok(Value::Float(a - b)),
        _ => Err(VmError::Type(
            "subtraction requires matching numbers".into(),
        )),
    }
}
fn mul(a: Value, b: Value) -> Result<Value, VmError> {
    match (a, b) {
        (Value::Int(a), Value::Int(b)) => {
            Ok(Value::Int(a.checked_mul(b).ok_or(VmError::Overflow)?))
        }
        (Value::Float(a), Value::Float(b)) => Ok(Value::Float(a * b)),
        _ => Err(VmError::Type(
            "multiplication requires matching numbers".into(),
        )),
    }
}
fn div(a: Value, b: Value) -> Result<Value, VmError> {
    match (a, b) {
        ...
        (Value::Int(a), Value::Int(b)) => {
            Ok(Value::Int(a.checked_div(b).ok_or(VmError::Overflow)?))
        }
        (Value::Float(a), Value::Float(b)) => Ok(Value::Float(a / b)),
        _ => Err(VmError::Type("division requires matching numbers".into())),
    }
}
fn modulo(a: Value, b: Value) -> Result<Value, VmError> {
    match (a, b) {
        (_, Value::Int(0)) => Err(VmError::DivisionByZero),
        (Value::Int(a), Value::Int(b)) => {
            Ok(Value::Int(a.checked_rem(b).ok_or(VmError::Overflow)?))
        }
        _ => Err(VmError::Type("modulo requires integers".into())),
    }
}
fn make_range(args: Vec<Value>) -> Result<Value, VmError> {
    let [Value::Int(start), Value::Int(end), Value::Bool(inclusive)] = args.as_slice() else {
        return Err(VmError::Type("range requires two integers".into()));
    };
    let stop = if *inclusive {
        end.checked_add(1).ok_or(VmError::Overflow)?
    } else {
        *end
    };
    ...
    if length > 1_000_000 {
        return Err(VmError::Type(
            "range exceeds the one-million element safety limit".into(),
        ));
    }
...
mod tests {
    ...
    fn compile(source: &str) -> Result<CompiledModule, String> {
    ...
    fn arithmetic_returns_value() {
        assert_eq!(run("fn main() { 40 + 2 }").unwrap(), Value::Int(42));
    }
    #[test]
    fn recursion_works() {
        assert_eq!(run("fn fact(n: int) -> int { if n <= 1 { return 1 } n * fact(n-1) } fn main() { fact(5) }").unwrap(), Value::Int(120));
    }
    #[test]
    ...
    fn native_text_and_encoding_work() {
        assert_eq!(
            ...
            Value::Str("natiT".into())
        );
        assert_eq!(run("fn main() { std::encoding::utf8_decode(std::encoding::base64_decode(\"VGl0YW4=\")) }").unwrap(), Value::Str("Titan".into()));
    }
    #[test]
    fn empty_map_constructor_and_length_work() {
        assert_eq!(
            run("fn main() { let values = std::map::new() std::map::length(values) }").unwrap(),
            Value::Int(0)
        );
    }
    #[test]
    fn persistent_map_insert_new_adds_entry() {
        assert_eq!(run("fn main() { let values = std::map::insert_new(std::map::new(), \"answer\", 42) std::map::length(values) }").unwrap(), Value::Int(1));
    }
    #[test]
    fn map_lookup_uses_string_content() {
        assert_eq!(run("fn main() { let values = std::map::insert_new(std::map::new(), \"answer\", 42) if std::map::contains(values, \"ans\" + \"wer\") { std::map::get(values, \"answer\") } else { 0 } }").unwrap(), Value::Int(42));
    }
    #[test]
    fn persistent_map_insert_replaces_by_content() {
        assert_eq!(run("fn main() { let first = std::map::insert(std::map::new(), \"answer\", 1) let updated = std::map::insert(first, \"ans\" + \"wer\", 42) std::map::length(updated) * 100 + std::map::get(updated, \"answer\") }").unwrap(), Value::Int(142));
    }
    #[test]
    fn persistent_map_remove_preserves_original() {
        assert_eq!(run("fn main() { let original = std::map::insert(std::map::new(), \"answer\", 42) let removed = std::map::remove(original, \"ans\" + \"wer\") std::map::length(original) * 100 + std::map::length(removed) }").unwrap(), Value::Int(100));
    }
    #[test]
    fn native_json_maps_support_fields() {
        assert_eq!(
            run(r#"fn main() { std::json::parse("{\"answer\":42}").answer }"#).unwrap(),
            Value::Int(42)
        );
    }
    #[test]
    fn interpolation_concatenates_every_segment() {
    ...
    #[test]
    fn closures_capture_lexical_values() {
        assert_eq!(
            run("fn main() { let base = 10 let add = |value: int| -> int value + base add(5) }")
                .unwrap(),
            Value::Int(15)
        );
    }
    #[test]
    fn named_functions_are_first_class() {
    ...
    #[test]
    fn persistent_array_set_returns_updated_copy() {
        assert_eq!(run("fn main() { let original = [10, 20] let updated = std::array::set(original, 1, 99) updated[1] }").unwrap(), Value::Int(99));
    }
    #[test]
    fn persistent_array_push_preserves_original() {
        assert_eq!(run("fn main() { let original = [10, 20] let updated = std::array::push(original, 30) len(original) * 100 + updated[2] }").unwrap(), Value::Int(230));
    }
    #[test]
    fn persistent_array_pop_handles_values_and_empty_arrays() {
        assert_eq!(run("fn main() { let original = [10, 20, 30] let shorter = std::array::pop(original) let empty = std::array::pop([]) len(original) * 100 + len(shorter) * 10 + len(empty) }").unwrap(), Value::Int(320));
    }
    #[test]
    fn persistent_array_slice_validates_and_copies_range() {
        assert_eq!(run("fn main() { let original = [10, 20, 30, 40] let part = std::array::slice(original, 1, 3) len(original) * 100 + part[0] + part[1] }").unwrap(), Value::Int(450));
    }
    #[test]
    fn persistent_array_concat_preserves_both_inputs() {
        assert_eq!(run("fn main() { let left = [1, 2] let right = [3, 4] let joined = std::array::concat(left, right) len(left) * 1000 + len(right) * 100 + len(joined) * 10 + joined[3] }").unwrap(), Value::Int(2244));
    }
    #[test]
    ...
    fn native_results_count_toward_the_memory_limit() {
        ...
        // arrays cost 768 bytes (charged via NewArray); their concat produces a
        // 20-element array that must also be charged, pushing the total past
        // the 800-byte limit.
        let module = compile(
    ...
    #[test]
    fn sandbox_blocks_effectful_natives() {
        let source = "fn main() { std::fs::read_text(\"secret\") }";
        let mut lexer = Lexer::new(source);
        let tokens = lexer.tokenize().0.to_vec();
        let program = Parser::new(tokens).parse_program().unwrap();
        let module = AstCompiler::new().compile_program(&program).unwrap();
        assert!(matches!(
            Vm::sandboxed(module).run(),
            Err(VmError::PermissionDenied { .. })
        ));
    }
    #[test]
    fn sandbox_denies_http_client_before_network_access() {
        let module = compile("fn main() { std::http::request(\"GET\", \"http://127.0.0.1:1/\", std::map::new(), std::encoding::utf8_encode(\"\"), 1, 0, 1) }").unwrap();
        assert!(
            matches!(Vm::sandboxed(module).run(), Err(VmError::PermissionDenied { function, capability }) if function == "std::http::request" && capability == "Network")
        );
    }
    #[test]
    fn sandbox_denies_persistent_readline_history_before_prompting() {
        let module =
            compile("fn main() { std::readline::prompt_persistent(\"prompt\", \"history.txt\") }")
                .unwrap();
        assert!(
            matches!(Vm::sandboxed(module).run(), Err(VmError::PermissionDenied { function, capability }) if function == "std::readline::prompt_persistent" && capability == "FilesystemUserInterface")
        );
    }
    #[test]
    fn sandbox_denies_user_interface_access_but_keeps_pure_geometry() {
        let module = compile("fn main() { std::clipboard::get_text() }").unwrap();
        assert!(
            matches!(Vm::sandboxed(module).run(), Err(VmError::PermissionDenied { function, capability }) if function == "std::clipboard::get_text" && capability == "UserInterface")
        );
        assert_eq!(
            run_sandboxed(
                "fn main() { std::game::check_collision(0.0, 0.0, 5.0, 5.0, 1.0, 1.0, 2.0, 2.0) }"
            )
            .unwrap(),
            Value::Bool(true)
        );
        ...
        assert!(
            matches!(Vm::sandboxed(module).run(), Err(VmError::PermissionDenied { function, capability }) if function == "std::dirs::current" && capability == "Environment")
        );
    }
    #[test]
    fn sandbox_denies_heap_dump_without_creating_a_file() {
        let path =
            std::env::temp_dir().join(format!("titan-sandbox-dump-{}.json", std::process::id()));
        let _ = std::fs::remove_file(&path);
        let escaped_path = path.display().to_string().replace('\\', "\\\\");
        let module = compile(&format!(
            "fn main() {{ std::runtime::heap_dump(\"{}\") }}",
            escaped_path
        ))
        .unwrap();
        assert!(
            matches!(Vm::sandboxed(module).run(), Err(VmError::PermissionDenied { function, capability }) if function == "std::runtime::heap_dump" && capability == "Filesystem")
        );
        assert!(!path.exists());
    }
    #[test]
    fn sandboxed_sqlite_memory_cannot_attach_a_file() {
        let path =
            std::env::temp_dir().join(format!("titan-sandbox-attach-{}.db", std::process::id()));
        let _ = std::fs::remove_file(&path);
        ...
        assert!(
            matches!(Vm::sandboxed(module).run(), Err(VmError::Native { function, .. }) if function == "std::sqlite")
        );
        assert!(!path.exists());
    }
    #[test]
    fn opaque_runtime_handles_are_owned_by_their_vm() {
        let module = compile("fn use_database(db: Sqlite) { std::sqlite::ping(db) } fn main() { std::sqlite::memory() }").unwrap();
        let function_id = module
            .functions
            .iter()
            .position(|function| function.name == "use_database")
            .unwrap();
        ...
        let mut debugger: Option<&mut dyn DebugHook> = None;
        assert!(
            matches!(foreign.execute(function_id, vec![handle], Vec::new(), 0, &mut debugger), Err(VmError::Type(message)) if message.contains("unknown SQLite connection"))
        );
    }
    #[test]
    fn owned_tasks_share_their_parent_runtime_handles() {
        assert_eq!(run("fn main() { let db = std::sqlite::memory() let task = spawn || std::sqlite::ping(db) join(task) }").unwrap(), Value::Bool(true));
    }
    #[cfg(feature = "kv_mod")]
    #[test]
    fn native_integer_handles_cannot_cross_vm_boundaries() {
        ...
        let read_id = module
            .functions
            .iter()
            .position(|function| function.name == "read_database")
            .unwrap();
        let close_id = module
            .functions
            .iter()
            .position(|function| function.name == "close_database")
            .unwrap();
        let mut owner = Vm::new(module.clone());
        let handle = owner.run().unwrap().unwrap();
        let mut debugger: Option<&mut dyn DebugHook> = None;
        assert!(owner
            .execute(read_id, vec![handle.clone()], Vec::new(), 0, &mut debugger)
            .is_ok());

        let mut foreign = Vm::new(module);
        let mut debugger: Option<&mut dyn DebugHook> = None;
        assert!(
            matches!(foreign.execute(read_id, vec![handle.clone()], Vec::new(), 0, &mut debugger), Err(VmError::Native { function, message }) if function == "std::kv::get" && message.contains("unknown database handle"))
        );

        let mut debugger: Option<&mut dyn DebugHook> = None;
        owner
            .execute(close_id, vec![handle], Vec::new(), 0, &mut debugger)
            .unwrap();
        let _ = std::fs::remove_dir_all(path);
    }
    #[cfg(feature = "kv_mod")]
    #[test]
    fn owned_tasks_share_native_integer_handles() {
    ...
    #[test]
    fn in_memory_native_handles_are_also_vm_owned() {
        ...
        let width_id = module
            .functions
            .iter()
            .position(|function| function.name == "image_width")
            .unwrap();
        let close_id = module
            .functions
            .iter()
            .position(|function| function.name == "close_image")
            .unwrap();
        let mut owner = Vm::new(module.clone());
        let handle = owner.run().unwrap().unwrap();
        let mut debugger: Option<&mut dyn DebugHook> = None;
        assert_eq!(
            owner
                .execute(width_id, vec![handle.clone()], Vec::new(), 0, &mut debugger)
                .unwrap(),
            Value::Int(1)
        );
        ...
        let mut debugger: Option<&mut dyn DebugHook> = None;
        assert!(
            matches!(foreign.execute(width_id, vec![handle.clone()], Vec::new(), 0, &mut debugger), Err(VmError::Native { function, message }) if function == "std::image::width" && message.contains("no image registered"))
        );
    ...
    #[test]
    fn debugger_breaks_steps_and_reports_state() {
        let mut lexer = Lexer::new("fn main() { let value = 40 value + 2 }");
        let tokens = lexer.tokenize().0.to_vec();
        let program = Parser::new(tokens).parse_program().unwrap();
        let module = AstCompiler::new().compile_program(&program).unwrap();
        let (controller, mut debugger) = Debugger::channel([Breakpoint::Instruction {
        ...
        let DebugEvent::Stopped(first) = controller.recv().unwrap() else {
            panic!("expected breakpoint")
        };
        assert_eq!(first.instruction, 0);
        controller.command(DebugCommand::StepIn).unwrap();
        let DebugEvent::Stopped(second) = controller.recv().unwrap() else {
            panic!("expected step")
        };
        assert!(second.instruction > first.instruction);
        controller.command(DebugCommand::Continue).unwrap();
        assert!(matches!(
            controller.recv().unwrap(),
            DebugEvent::Terminated { error: None }
        ));
        assert_eq!(thread.join().unwrap().unwrap(), Some(Value::Int(42)));
    }
    #[test]
    fn tasks_execute_on_threads_and_join() {
        assert_eq!(
            run("fn main() { let task = spawn || 40 + 2 join(task) }").unwrap(),
            Value::Int(42)
        );
    }
    #[test]
    fn channels_communicate_between_tasks() {
        assert_eq!(run("fn main() { let pair = channel(1) let tx = pair[0] let rx = pair[1] let task = spawn || { send(tx, 42) } let value = recv(rx) join(task) value }").unwrap(), Value::Int(42));
    }
    #[test]
    fn task_and_channel_timeouts_are_explicit_options() {
        assert_eq!(run("fn main() { let pair = channel(1) let result = recv_timeout(pair[1], 1) match result { Option::None => 1, Option::Some(value) => 0 } }").unwrap(), Value::Int(1));
    }
    #[test]
    fn select_returns_channel_index_and_value() {
        assert_eq!(run("fn main() { let a = channel(1) let b = channel(1) let task = spawn || { send(b[0], 77) } let selected = select([a[1], b[1]], 1000)? join(task) selected[1] }").unwrap(), Value::Int(77));
    }
    #[test]
    fn cancellation_stops_cooperative_tasks() {
        assert!(run("fn main() { let task = spawn || loop {} let pending = join_timeout(task, 1) cancel(task) join(task) }").is_err());
    }
    #[test]
    fn tcp_round_trip_works_across_tasks() {
        assert_eq!(run("fn main() { let listener = std::net::tcp_listen(\"127.0.0.1:0\") let address = std::net::tcp_local_addr(listener) let server = spawn || { let accepted = std::net::tcp_accept(listener) let stream = accepted[0] let request = std::net::tcp_read(stream, 16) std::net::tcp_write(stream, request) std::net::tcp_close(stream) } let client = std::net::tcp_connect(address) std::net::tcp_write(client, \"ping\") let response = std::net::tcp_read(client, 16) std::net::tcp_close(client) join(server) std::net::tcp_close(listener) std::encoding::utf8_decode(response) }").unwrap(), Value::Str("ping".into()));
    }
    #[test]
    fn http_codec_is_callable_from_titan() {
        assert_eq!(run("fn main() { let bytes = std::encoding::utf8_encode(\"GET /hello?name=titan HTTP/1.1\\r\\nHost: localhost\\r\\n\\r\\n\") let request = std::http::parse_request(bytes)? request.path }").unwrap(), Value::Str("/hello".into()));
        assert!(matches!(run("fn main() { let headers = std::json::parse(\"{\\\"Content-Type\\\":\\\"text/plain\\\"}\") std::http::build_response(200, headers, std::encoding::utf8_encode(\"ok\"), false) }").unwrap(), Value::Bytes(_)));
    }
    #[test]
    ...
    fn multipart_uploads_are_callable_from_titan() {
        assert_eq!(run("fn main(){let body=std::encoding::utf8_encode(\"--x\\r\\nContent-Disposition: form-data; name=\\\"title\\\"\\r\\n\\r\\nTitan\\r\\n--x--\\r\\n\") let parts=std::http::parse_multipart(\"multipart/form-data; boundary=x\",body,4,1024) std::encoding::utf8_decode(parts[0].data)}").unwrap(),Value::Str("Titan".into()));
    }
    #[test]
    fn metrics_are_callable_from_titan() {
        assert_eq!(run("fn main(){std::metrics::reset() std::metrics::counter_add(\"requests.total\",2) std::metrics::histogram_record(\"request.ms\",12.5) let metrics=std::metrics::snapshot() std::map::get(metrics.counters,\"requests.total\")}").unwrap(),Value::Int(2));
    }
    #[test]
    fn game_engine_surface_is_callable_from_titan() {
        assert_eq!(
            run("fn main(){std::game::init(\"Titan QA\",64,64) std::game::shutdown()}").unwrap(),
            Value::Bool(true)
        );
    }
    #[test]
    fn gui_render_produces_real_rgba_bytes_from_titan() {
        assert_eq!(run("fn main(){std::gui::init() let root=std::gui::create_container(\"QA\",20,20) std::bytes::length(std::gui::render(root))}").unwrap(),Value::Int(1600));
    }
    #[test]
    fn gui_raster_feeds_image_pipeline_from_titan() {
        assert_eq!(run("fn main(){std::gui::init() let root=std::gui::create_container(\"QA\",8,8) let img=std::image::from_rgba(8,8,std::gui::render(root)) std::image::width(img)}").unwrap(),Value::Int(8));
    }
    #[test]
    fn input_setters_feed_readers_from_titan() {
        assert_eq!(run("fn main(){std::input::set_key_state(\"W\",true) std::input::is_key_pressed(\"W\")}").unwrap(),Value::Bool(true));
    }
    #[cfg(all(feature = "window_live", not(target_os = "android")))]
    #[test]
    fn live_window_services_reject_unknown_handles_from_titan() {
        assert_eq!(
            run("fn main(){std::window::live_pump(424242,1)}").unwrap(),
            Value::Int(-2)
        );
        assert_eq!(
            run("fn main(){std::window::live_set_title(424242,\"ghost\")}").unwrap(),
            Value::Bool(false)
        );
    }
    #[cfg(all(feature = "window_live", not(target_os = "android")))]
    #[test]
    fn live_window_events_are_empty_for_unknown_handles_from_titan() {
        assert_eq!(
            run("fn main(){len(std::window::live_poll_events(424242))}").unwrap(),
            Value::Int(0)
        );
    }
    #[test]
    fn input_touch_round_trip_from_titan() {
        assert_eq!(
    ...
    fn sqlite_pool_leases_are_reused_from_titan() {
        let path = std::env::temp_dir().join(format!("titan-vm-pool-{}.db", std::process::id()));
        let _ = std::fs::remove_file(&path);
        let escaped_path = path.display().to_string().replace('\\', "\\\\");
    ...
    #[test]
    fn sqlite_ping_and_pool_health_work_from_titan() {
        let path = std::env::temp_dir().join(format!("titan-vm-health-{}.db", std::process::id()));
        let _ = std::fs::remove_file(&path);
        let escaped_path = path.display().to_string().replace('\\', "\\\\");
    ...
    #[test]
    fn common_database_api_works_with_sqlite() {
        assert_eq!(run("fn main(){let db=std::sqlite::memory() std::db::execute(db,\"CREATE TABLE values_(value INTEGER)\",[]) std::db::begin(db) std::db::execute(db,\"INSERT INTO values_ VALUES (?)\",[42]) std::db::commit(db) let rows=std::db::query(db,\"SELECT value FROM values_\",[]) std::db::close(db) rows[0].value}").unwrap(),Value::Int(42));
    }
    #[test]
    fn sqlite_prepared_queries_and_transactions_work() {
    ...
    fn http_routing_is_callable_from_titan() {
        assert_eq!(run("fn main() { let params = std::http::route_match(\"/users/:id\", \"/users/42\")? params.id }").unwrap(), Value::Str("42".into()));
        assert_eq!(run("fn main() { let query = std::http::parse_query(\"tag=rust&tag=titan\", 10) query.tag[1] }").unwrap(), Value::Str("titan".into()));
    }
    #[test]
    fn composed_router_dispatches_middleware_and_params() {
        assert_eq!(run("fn main(){let router=std::http::router() std::http::middleware(router,|request|request) std::http::route(router,\"GET\",\"/users/:id\",|request|request.params.id) let request=std::json::parse(\"{\\\"method\\\":\\\"GET\\\",\\\"path\\\":\\\"/users/42\\\"}\") std::http::dispatch(router,request)}").unwrap(),Value::Str("42".into()));
        assert!(matches!(run("fn main(){let router=std::http::router() let request=std::json::parse(\"{\\\"method\\\":\\\"GET\\\",\\\"path\\\":\\\"/missing\\\"}\") std::http::dispatch(router,request)}").unwrap(),Value::Map(_)));
    }
    #[test]
    fn websocket_codec_is_callable_from_titan() {
        assert_eq!(run("fn main(){let frame=std::ws::encode(1,std::encoding::utf8_encode(\"hello\"),true) let parsed=std::ws::parse(frame,true,1024)? std::encoding::utf8_decode(parsed.payload)}").unwrap(),Value::Str("hello".into()));
        assert_eq!(
            run("fn main(){std::ws::accept_key(\"dGhlIHNhbXBsZSBub25jZQ==\")}").unwrap(),
            Value::Str("s3pPLMBiTxaQ9kYGzzhZRbK+xOo=".into())
        );
    }
    #[test]
    fn websocket_http_upgrade_is_validated_from_titan() {
        assert_eq!(run("fn main(){let key=\"dGhlIHNhbXBsZSBub25jZQ==\" let raw=std::encoding::utf8_encode(\"GET /chat HTTP/1.1\\r\\nHost: localhost\\r\\nUpgrade: websocket\\r\\nConnection: keep-alive, Upgrade\\r\\nSec-WebSocket-Version: 13\\r\\nSec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\\r\\nSec-WebSocket-Protocol: chat, superchat\\r\\n\\r\\n\") let request=std::http::parse_request(raw)? let response=std::ws::validate_upgrade(request,\"chat\") std::ws::validate_accept(response,key)}").unwrap(),Value::Bool(true));
    }
    #[test]
    fn stateful_websocket_decoder_is_callable_from_titan() {
    ...
    fn websocket_client_performs_automatic_http_upgrade() {
        assert_eq!(run("fn main(){let listener=std::net::tcp_listen(\"127.0.0.1:0\") let address=std::net::tcp_local_addr(listener) let server=spawn || {let accepted=std::net::tcp_accept(listener) let stream=accepted[0] let raw=std::net::tcp_read(stream,65536) let request=std::http::parse_request(raw)? let upgrade=std::ws::validate_upgrade(request,\"\") std::net::tcp_write(stream,upgrade) let ws=std::ws::attach_tcp(stream,true,1024) let message=std::ws::receive(ws) std::ws::send_text(ws,message.text) std::ws::close(ws,1000,\"done\")} let client=std::ws::connect(\"ws://\"+address+\"/echo\",\"\",1024) std::ws::send_text(client,\"automatic\") let response=std::ws::receive(client) std::ws::close(client,1000,\"done\") join(server) std::net::tcp_close(listener) response.text}").unwrap(),Value::Str("automatic".into()));
    }
    #[test]
    fn router_recovers_handler_errors_as_http_responses() {
        assert_eq!(run("fn main(){let router=std::http::router() std::http::route(router,\"GET\",\"/fail\",|request|1/0) std::http::on_error(router,|request,error|std::http::error_response(500,error.message)) let request=std::json::parse(\"{\\\"method\\\":\\\"GET\\\",\\\"path\\\":\\\"/fail\\\"}\") let response=std::http::dispatch(router,request) response.status}").unwrap(),Value::Int(500));
        assert!(matches!(
            run("fn main(){std::http::json_response(201,std::json::parse(\"{\\\"ok\\\":true}\"))}")
                .unwrap(),
            Value::Map(_)
        ));
    }
    ...
    #[test]
    fn sandbox_denies_tcp_access() {
        assert!(run_sandboxed("fn main() { std::net::tcp_listen(\"127.0.0.1:0\") }").is_err());
        assert!(
            run_sandboxed("fn main() { std::tls::connect(\"127.0.0.1:443\",\"localhost\") }")
                .is_err()
        );
        assert!(run_sandboxed(
            "fn main() { std::postgres::connect(\"postgresql://localhost/test\") }"
        )
        .is_err());
        assert!(run_sandboxed(
            "fn main() { std::postgres::connect_tls(\"postgresql://localhost/test\") }"
        )
        .is_err());
        assert!(run_sandboxed(
            "fn main() { std::postgres::pool(\"postgresql://localhost/test\",4,true) }"
        )
        .is_err());
        assert!(
            run_sandboxed("fn main() { std::mysql::connect(\"mysql://localhost/test\") }").is_err()
        );
        assert!(
            run_sandboxed("fn main() { std::mysql::pool(\"mysql://localhost/test\",4) }").is_err()
        );
    }
    #[test]
    fn runtime_errors_are_reported() {
        assert!(run("fn main() { 1 / 0 }").is_err());
    }
}
