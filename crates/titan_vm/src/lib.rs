...
pub enum Value {
    ...
    TcpListener(u64), TcpStream(u64), HttpRouter(u64), TlsStream(u64), TlsServerConfig(u64), WebSocketDecoder(u64), WebSocket(u64), ServerControl(u64), Sqlite(u64), SqlitePool(u64), Postgres(u64),
}
...
struct RuntimeState {
    ...
    postgres: Mutex<HashMap<u64, Arc<Mutex<titan_postgres::Database>>>>,
}
    ...
    fn execute(&mut self, function_id: usize, args: Vec<Value>, captures: Vec<Value>, depth: usize, debugger: &mut Option<&mut dyn DebugHook>) -> Result<Value, VmError> {
        ...
        while ip < function.code.len() {
            ...
            match function.code[ip].clone() {
                ...
                Op::PostgresConnect => { require_network(self.capabilities,"std::postgres::connect")?;let Value::Str(url)=pop(&mut stack,&function.name)? else{return Err(VmError::Type("PostgreSQL URL must be string".into()))};let database=titan_postgres::Database::connect(&url).map_err(postgres_error)?;let id=self.runtime.next_socket.fetch_add(1,Ordering::Relaxed);self.runtime.postgres.lock().map_err(|_|VmError::Type("PostgreSQL registry poisoned".into()))?.insert(id,Arc::new(Mutex::new(database)));stack.push(Value::Postgres(id)); }
...
mod tests {
    ...
    #[test] fn sqlite_pool_leases_are_reused_from_titan(){let path=std::env::temp_dir().join(format!("titan-vm-pool-{}.db",std::process::id()));let _=std::fs::remove_file(&path);let source=format!("fn main(){{let pool=std::sqlite::pool(\"{}\",1) let first=std::sqlite::acquire(pool,1000)? let pending=std::sqlite::acquire(pool,1) std::sqlite::close(first) let second=std::sqlite::acquire(pool,1000)? let stats=std::sqlite::pool_stats(pool) std::sqlite::close(second) std::sqlite::pool_close(pool); [pending,stats.total]}}",path.display());assert_eq!(run(&source).unwrap(),Value::Array(vec![Value::Enum{name:"Option".into(),variant:"None".into(),payload:None},Value::Int(1)]));let _=std::fs::remove_file(path);}
    ...
    #[test] fn sandbox_denies_tcp_access() { assert!(run_sandboxed("fn main() { std::net::tcp_listen(\"127.0.0.1:0\") }").is_err()); assert!(run_sandboxed("fn main() { std::tls::connect(\"127.0.0.1:443\",\"localhost\") }").is_err()); assert!(run_sandboxed("fn main() { std::postgres::connect(\"postgresql://localhost/test\") }").is_err()); }
