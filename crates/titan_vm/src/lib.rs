            ...
            match function.code[ip].clone() {
                ...
                Op::HttpServeConnection => {
                    ...
                    'requests: for _ in 0..maximum { let request = loop { if let Some((request, consumed)) = titan_stdlib::http::parse_request(&buffer, &titan_stdlib::http::HttpLimits::default()).map_err(|error| VmError::Native { function: "std::http::serve_connection".into(), message: error.to_string() })? { buffer.drain(..consumed); break request; } let mut chunk = [0u8; 8192]; let count = stream.read(&mut chunk).map_err(|error| network_error("std::http::serve_connection", error))?; if count == 0 { break 'requests; } buffer.extend_from_slice(&chunk[..count]); if buffer.len() > 16 * 1024 * 1024 + 64 * 1024 { return Err(VmError::Type("HTTP connection buffer limit exceeded".into())); } };
                        let request_keep_alive = request.keep_alive; let response_value = self.execute(handler_function, vec![http_request_value(request, &peer.to_string())], handler_captures.clone(), depth + 1, debugger)?; let (status, headers, body, keep_alive) = http_response_value(response_value, request_keep_alive)?; let response = titan_stdlib::http::build_response(status, &headers, &body, keep_alive).map_err(|error| VmError::Native { function: "std::http::serve_connection".into(), message: error.to_string() })?; stream.write_all(&response).map_err(|error| network_error("std::http::serve_connection", error))?; if !keep_alive { break; }
                    }
...
fn http_response_value(value: Value, request_keep_alive: bool) -> Result<(u16,BTreeMap<String,String>,Vec<u8>,bool),VmError> { let Value::Map(mut map)=value else{return Err(VmError::Type("HTTP handler must return a response map".into()))};let status=match map.remove("status").unwrap_or(Value::Int(200)){Value::Int(status)=>u16::try_from(status).map_err(|_|VmError::Type("HTTP response status out of range".into()))?,_=>return Err(VmError::Type("HTTP response status must be int".into()))};let headers=match map.remove("headers").unwrap_or(Value::Map(BTreeMap::new())){Value::Map(headers)=>headers.into_iter().map(|(key,value)|if let Value::Str(value)=value{Ok((key,value))}else{Err(VmError::Type("HTTP response headers must be strings".into()))}).collect::<Result<_,_>>()?,_=>return Err(VmError::Type("HTTP response headers must be a map".into()))};let body=match map.remove("body").unwrap_or(Value::Bytes(Vec::new())){Value::Bytes(body)=>body,Value::Str(body)=>body.into_bytes(),_=>return Err(VmError::Type("HTTP response body must be bytes or string".into()))};let keep_alive=match map.remove("keep_alive").unwrap_or(Value::Bool(request_keep_alive)){Value::Bool(value)=>value&&request_keep_alive,_=>return Err(VmError::Type("HTTP response keep_alive must be bool".into()))};Ok((status,headers,body,keep_alive)) }
