    ...
    fn execute(&mut self, function_id: usize, args: Vec<Value>, captures: Vec<Value>, depth: usize, debugger: &mut Option<&mut dyn DebugHook>) -> Result<Value, VmError> {
        ...
        while ip < function.code.len() {
            ...
            match function.code[ip].clone() {
                ...
                Op::WsConnect => { require_network(self.capabilities,"std::ws::connect")?;let maximum=positive_limit(pop(&mut stack,&function.name)?,"WebSocket maximum")?;let Value::Str(protocol)=pop(&mut stack,&function.name)? else{return Err(VmError::Type("WebSocket protocol must be string".into()))};let Value::Str(url)=pop(&mut stack,&function.name)? else{return Err(VmError::Type("WebSocket URL must be string".into()))};let parsed=titan_stdlib::websocket::parse_url(&url).map_err(|error|VmError::Native{function:"std::ws::connect".into(),message:error.to_string()})?;let address=if parsed.host.contains(':'){format!("[{}]:{}",parsed.host,parsed.port)}else{format!("{}:{}",parsed.host,parsed.port)};let transport=if parsed.secure{WebSocketTransport::Tls(Arc::new(Mutex::new(titan_tls::connect(&address,&parsed.host,titan_tls::client_config()).map_err(|error|VmError::Native{function:"std::ws::connect".into(),message:error.to_string()})?)))}else{let socket=TcpStream::connect(&address).map_err(|error|network_error("std::ws::connect",error))?;socket.set_read_timeout(Some(Duration::from_secs(10))).and_then(|_|socket.set_write_timeout(Some(Duration::from_secs(10)))).map_err(|error|network_error("std::ws::connect",error))?;WebSocketTransport::Tcp(Arc::new(Mutex::new(socket)))};let key=titan_stdlib::websocket::client_key().map_err(|error|VmError::Native{function:"std::ws::connect".into(),message:error.to_string()})?;let mut request=format!("GET {} HTTP/1.1\r\nHost: {}\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Version: 13\r\nSec-WebSocket-Key: {}\r\n",parsed.path,parsed.authority,key);if !protocol.is_empty(){request.push_str(&format!("Sec-WebSocket-Protocol: {protocol}\r\n"));}request.push_str("\r\n");websocket_transport_write(&transport,request.as_bytes())?;let mut response=Vec::new();loop{if response.windows(4).any(|window|window==b"\r\n\r\n"){break}if response.len()>64*1024{return Err(VmError::Type("WebSocket handshake headers too large".into()))}let mut chunk=[0u8;4096];let count=websocket_transport_read(&transport,&mut chunk)?;if count==0{return Err(VmError::WebSocketDisconnected)}response.extend_from_slice(&chunk[..count]);}let consumed=titan_stdlib::websocket::validate_accept_response(&response,&key).map_err(|error|VmError::Native{function:"std::ws::connect".into(),message:error.to_string()})?;titan_stdlib::websocket::validate_subprotocol_response(&response,&protocol).map_err(|error|VmError::Native{function:"std::ws::connect".into(),message:error.to_string()})?;let id=insert_websocket(&self.runtime,transport,false,maximum)?;if response.len()>consumed{let socket=pop_websocket(&self.runtime,Value::WebSocket(id))?;socket.decoder.lock().map_err(|_|VmError::Type("WebSocket decoder poisoned".into()))?.push(&response[consumed..]).map_err(|error|VmError::Native{function:"std::ws::connect".into(),message:error.to_string()})?;}stack.push(Value::WebSocket(id)); }
...
mod tests {
    ...
    #[test] fn sandbox_denies_tcp_access() { assert!(run_sandboxed("fn main() { std::net::tcp_listen(\"127.0.0.1:0\") }").is_err()); assert!(run_sandboxed("fn main() { std::tls::connect(\"127.0.0.1:443\",\"localhost\") }").is_err()); }
