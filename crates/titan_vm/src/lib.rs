...
mod tests {
    ...
    #[test] fn sandbox_denies_tcp_access() { assert!(run_sandboxed("fn main() { std::net::tcp_listen(\"127.0.0.1:0\") }").is_err()); assert!(run_sandboxed("fn main() { std::tls::connect(\"127.0.0.1:443\",\"localhost\") }").is_err()); assert!(run_sandboxed("fn main() { std::postgres::connect(\"postgresql://localhost/test\") }").is_err()); assert!(run_sandboxed("fn main() { std::postgres::connect_tls(\"postgresql://localhost/test\") }").is_err()); }
