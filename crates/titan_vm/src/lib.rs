...
pub enum Value {
    ...
    TcpListener(u64), TcpStream(u64), HttpRouter(u64), TlsStream(u64), TlsServerConfig(u64),
}
...
pub fn val_to_string(value: &Value) -> String {
    match value {
        ...
        Value::Task(id) => format!("<task:{id}>"), Value::ChannelSender(id) => format!("<sender:{id}>"), Value::ChannelReceiver(id) => format!("<receiver:{id}>"), Value::TcpListener(id) => format!("<tcp-listener:{id}>"), Value::TcpStream(id) => format!("<tcp-stream:{id}>"), Value::HttpRouter(id) => format!("<http-router:{id}>"), Value::TlsStream(id) => format!("<tls-stream:{id}>"), Value::TlsServerConfig(id) => format!("<tls-server-config:{id}>"),
    }
}
