...
fn to_json(value: Value) -> Result<serde_json::Value, String> {
    Ok(match value {
        ...
        Value::Closure { .. } | Value::Task(_) | Value::ChannelSender(_) | Value::ChannelReceiver(_) | Value::TcpListener(_) | Value::TcpStream(_) => return Err("runtime handles cannot be encoded as JSON".into()),
    })
}
