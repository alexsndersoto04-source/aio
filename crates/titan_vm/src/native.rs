...
fn dispatch(name: &str, mut args: Vec<Value>) -> Result<Value, String> {
    ...
    Ok(match name {
        ...
        "std::metrics::counter_get" => Value::Int(i64::try_from(stdlib::metrics::counter_get(&string!()).map_err(error)?).unwrap_or(i64::MAX)),
        "std::metrics::gauge_set" => {let name=string!();let value=float!();stdlib::metrics::gauge_set(&name,value).map_err(error)?;Value::Nil}
        "std::metrics::gauge_get" => Value::Float(stdlib::metrics::gauge_get(&string!()).map_err(error)?),
        ...
        "std::metrics::prometheus_export" => Value::Str(stdlib::metrics::prometheus_export().map_err(error)?),
