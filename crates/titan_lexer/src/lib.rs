...
impl Lexer {
    ...
    fn skip_trivia(&mut self) {
        loop {
            ...
            if self.peek() == Some('/') && self.peek_n(1) == Some('*') {
                ...
                if depth != 0 {
                    self.errors
                        .push(LexerError::UnterminatedComment { line, column });
                }
                continue;
            }
            break;
    ...
    fn next_token(&mut self) -> Token {
        ...
        let kind = match c {
            ...
            '#' => {
                if self.eat('{') {
                    ...
                    TokenKind::Error("stray '#' — did you mean '#{'?".into())
                }
            }
            '?' => TokenKind::Question,
            '!' => {
            ...
            character => {
                self.errors.push(LexerError::InvalidCharacter {
                    character,
                    line,
                    column,
                });
        ...
        Token {
            kind,
            span: Span::new(start, self.byte_offset(self.pos), line, column),
        }
    }
    ...
    fn lex_string(&mut self, line: usize, column: usize) -> TokenKind {
        ...
        while let Some(c) = self.peek() {
            match c {
                '"' => {
                    self.advance();
                    return TokenKind::StringLit(value);
                }
                '\n' => {
                    self.errors
                        .push(LexerError::UnterminatedString { line, column });
                    ...
                    match self.advance() {
                        ...
                        Some(escape) => {
                            self.errors.push(LexerError::InvalidEscape {
                                escape,
                                line: self.line,
                                column: self.col.saturating_sub(1),
                            });
                            value.push(escape);
                        }
                        None => break,
    ...
    fn lex_char(&mut self, line: usize, column: usize) -> TokenKind {
        let value = match self.advance() {
            Some('\\') => match self.advance() {
                ...
                None => {
                    self.errors
                        .push(LexerError::UnterminatedChar { line, column });
