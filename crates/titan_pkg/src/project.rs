...
impl SourceProject {
    pub fn load(entry: impl AsRef<Path>) -> Result<Self, ProjectError> {
        ...
        if !requested.exists() {
            return Err(ProjectError::EntryNotFound(requested.to_path_buf()));
        }
        let entry = requested
            .canonicalize()
        ...
        let source_root_candidate = if root.join("Titan.toml").is_file() {
            root.join("src")
        } else {
            root.clone()
        };
        let source_root =
            source_root_candidate
                .canonicalize()
                .map_err(|source| ProjectError::Read {
                    path: source_root_candidate,
                    source,
                })?;
        let manifest = if root.join("Titan.toml").is_file() {
            Some(
                crate::Manifest::from_dir(&root).map_err(|error| ProjectError::Manifest {
                    path: root.join("Titan.toml"),
        ...
        let dependencies = if let Some(manifest) = &manifest {
            collect_dependencies(&root, manifest)?
        } else {
            BTreeMap::new()
        };
        let mut allowed_roots = vec![source_root.clone()];
        allowed_roots.extend(dependencies.values().cloned());
        ...
        let mut loader = Loader {
            ...
            dependency_roots: dependencies.clone(),
            allowed_roots,
...
impl Loader {
    fn visit(&mut self, path: &Path) -> Result<(), ProjectError> {
        let path = path.canonicalize().map_err(|source| ProjectError::Read {
        ...
        if let Some(position) = self.stack.iter().position(|item| item == &path) {
            let mut cycle: Vec<_> = self.stack[position..]
                .iter()
                .map(|p| relative_display(p, &self.source_root))
                .collect();
            cycle.push(relative_display(&path, &self.source_root));
            return Err(ProjectError::ImportCycle {
                chain: cycle.join(" -> "),
            });
        }
        ...
        let source = std::fs::read_to_string(&path).map_err(|source| ProjectError::Read {
            path: path.clone(),
            source,
        })?;
    ...
    fn resolve_import(&self, segments: &[String], from: &Path) -> Result<PathBuf, ProjectError> {
        ...
        let (base, path_segments) = if let Some(root) = segments
            .first()
            .and_then(|name| self.dependency_roots.get(name))
        {
            (root, &segments[1..])
        } else {
            let owner = self
                .allowed_roots
                .iter()
                .filter(|root| from.starts_with(root))
        ...
        for length in (1..=path_segments.len()).rev() {
            let relative = path_segments[..length]
                .iter()
            ...
            for candidate in candidates {
                if candidate.is_file() {
                    ...
                    {
                        return Err(ProjectError::ImportEscapesRoot {
                            import: display,
                            root: base.clone(),
                        });
                    }
...
fn collect_dependencies(
...
) -> Result<BTreeMap<String, PathBuf>, ProjectError> {
    let remote_versions: BTreeMap<String, String> = if root.join("Titan.remote.lock").is_file() {
        crate::RemoteLockfile::read(&root.join("Titan.remote.lock"))
            .map_err(|error| ProjectError::Manifest {
                path: root.join("Titan.remote.lock"),
                message: error.to_string(),
            })?
            .packages
            .into_iter()
    ...
    fn visit(
        root: &Path,
        project_root: &Path,
        remote_versions: &BTreeMap<String, String>,
        manifest: &crate::Manifest,
        output: &mut BTreeMap<String, PathBuf>,
        stack: &mut Vec<PathBuf>,
    ) -> Result<(), ProjectError> {
        let root = root.canonicalize().map_err(|source| ProjectError::Read {
        ...
        for (alias, dependency) in &manifest.dependencies {
            let dependency_candidate = if let Some(relative) = &dependency.path {
                root.join(relative)
            } else {
                let version = remote_versions.get(alias).ok_or_else(|| {
                    ProjectError::DependencyNeedsPath {
                        name: alias.clone(),
                    }
                })?;
                project_root
            ...
            let dependency_root =
                dependency_candidate
                    .canonicalize()
                    .map_err(|source| ProjectError::Read {
                        path: dependency_candidate,
                        source,
                    })?;
            let dependency_manifest =
                crate::Manifest::from_dir(&dependency_root).map_err(|error| {
                    ProjectError::Manifest {
                        path: dependency_root.join("Titan.toml"),
                        message: error.to_string(),
                    }
                })?;
        ...
        match item {
            ...
            Item::Impl(block) => {
                for method in &mut block.methods {
                    method.source_file = Some(source.clone());
                }
            }
...
pub fn find_project_root(path: &Path) -> Option<PathBuf> {
    ...
    start
        .ancestors()
        .find(|directory| directory.join("Titan.toml").is_file())
        .map(Path::to_path_buf)
}

pub fn default_entry(path: impl AsRef<Path>) -> PathBuf {
    let path = path.as_ref();
    if path.is_file() || path.extension().and_then(|value| value.to_str()) == Some("titan") {
        return path.to_path_buf();
    }
    if let Some(root) = find_project_root(path) {
        return root.join("src/main.titan");
    }
...
pub fn create_project(path: impl AsRef<Path>, name: &str) -> Result<PathBuf, ProjectError> {
    ...
    std::fs::create_dir_all(&source_dir).map_err(|error| ProjectError::Create {
        path: root.to_path_buf(),
        source: error,
    })?;
    let manifest = format!("[package]\nname = \"{name}\"\nversion = \"0.1.0\"\nedition = \"2021\"\nlicense = \"MIT\"\n\n[dependencies]\n");
    std::fs::write(root.join("Titan.toml"), manifest).map_err(|error| ProjectError::Create {
        path: root.join("Titan.toml"),
        source: error,
    })?;
    ...
    .map_err(|error| ProjectError::Create {
        path: main_source,
        source: error,
    })?;
    Ok(root.to_path_buf())
...
mod tests {
    ...
    fn loads_imports_once_in_dependency_order() {
        let root = temporary("imports");
        create_project(&root, "imports").unwrap();
        std::fs::write(
            root.join("src/math.titan"),
        ...
        assert_eq!(project.program.items.len(), 2);
        assert!(project
            .program
            .items
            .iter()
            .filter_map(|item| if let Item::Function(function) = item {
                function.source_file.as_ref()
            } else {
                None
            })
            .all(|source| source.ends_with(".titan")));
    ...
    fn detects_import_cycles() {
        ...
        create_project(&root, "cycle").unwrap();
        ...
        assert!(matches!(
            SourceProject::load(root.join("src/main.titan")),
            Err(ProjectError::ImportCycle { .. })
        ));
        std::fs::remove_dir_all(root).unwrap();
    }
    ...
    fn resolves_local_path_dependencies() {
        let parent = temporary("dependency");
        let dependency = parent.join("mathlib");
        let application = parent.join("app");
        create_project(&dependency, "mathlib").unwrap();
        std::fs::rename(
            dependency.join("src/main.titan"),
            dependency.join("src/lib.titan"),
        )
        .unwrap();
        std::fs::write(
            dependency.join("src/lib.titan"),
        ...
        std::fs::write(
            application.join("src/main.titan"),
            "import math\nfn main() { triple(14) }",
        )
        .unwrap();
        let project = SourceProject::load(application.join("src/main.titan")).unwrap();
        assert!(project.dependencies.contains_key("math"));
        assert_eq!(project.program.items.len(), 2);
