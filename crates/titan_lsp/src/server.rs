    ...
    while let Some(message) = read_message(&mut input)? {
        ...
        if let Some(id) = id {
            let result = match method {
                "initialize" => Ok(json!({ "capabilities": { "textDocumentSync": { "openClose": true, "change": 2 }, "completionProvider": { "triggerCharacters": [".", ":"] }, "hoverProvider": true, "definitionProvider": true, "referencesProvider": true, "renameProvider": { "prepareProvider": false }, "documentSymbolProvider": true, "workspaceSymbolProvider": true }, "serverInfo": { "name": "titan-lsp", "version": env!("CARGO_PKG_VERSION") } })),
