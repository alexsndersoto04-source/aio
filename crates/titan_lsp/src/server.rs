...
impl<R: Read> Read for BufReadAdapter<R> {
    fn read(&mut self, buffer: &mut [u8]) -> io::Result<usize> {
        self.0.read(buffer)
    }
}
impl<R: BufRead> BufRead for BufReadAdapter<R> {
            ...
            match method {
                ...
                "textDocument/didOpen" => {
                    if let Some(document) = params.get("textDocument") {
                        ...
                        publish(&mut output, &service, uri)?;
                    }
                }
                "textDocument/didChange" => {
                    let document = params
                        .get("textDocument")
                        .ok_or_else(|| ServerError::Frame("missing textDocument".into()))?;
                    let uri = string(document, "uri")?;
                        ...
                        .flatten()
                    {
                        ...
                        service
                            .apply_change(uri, range, string(change, "text")?, version)
                            .map_err(ServerError::Frame)?;
                    }
                    publish(&mut output, &service, uri)?;
                }
                "textDocument/didClose" => {
                    let uri = document_uri(&params).map_err(ServerError::Frame)?;
...
fn publish(output: &mut impl Write, service: &TitanLsp, uri: &str) -> Result<(), ServerError> {
    write_message(
        output,
        &json!({"jsonrpc":"2.0","method":"textDocument/publishDiagnostics","params":{"uri":uri,"diagnostics":service.diagnostics(uri)}}),
    )
}
fn document_uri(params: &Value) -> Result<&str, String> {
    params
        .get("textDocument")
...
fn document_position(params: &Value) -> Result<(&str, Position), String> {
    let uri = document_uri(params)?;
    let position = params.get("position").ok_or("missing position")?;
    Ok((uri, parse_position(position)?))
}
...
fn parse_range(value: &Value) -> Result<Range, String> {
    Ok(Range {
        start: parse_position(value.get("start").ok_or("missing range.start")?)?,
        end: parse_position(value.get("end").ok_or("missing range.end")?)?,
    })
}
fn string<'a>(value: &'a Value, name: &str) -> Result<&'a str, ServerError> {
    value
        .get(name)
        .and_then(Value::as_str)
        .ok_or_else(|| ServerError::Frame(format!("missing {name}")))
}
...
fn read_message(input: &mut impl BufRead) -> Result<Option<Value>, ServerError> {
    ...
    let length =
        content_length.ok_or_else(|| ServerError::Frame("missing Content-Length".into()))?;
...
mod tests {
    use super::*;
    #[test]
    fn serves_initialize_open_and_symbols_over_framed_json_rpc() {
        let messages = [
            json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}),
            json!({"jsonrpc":"2.0","method":"textDocument/didOpen","params":{"textDocument":{"uri":"file:///x.titan","version":1,"text":"fn main() { 1 }"}}}),
            json!({"jsonrpc":"2.0","id":2,"method":"textDocument/documentSymbol","params":{"textDocument":{"uri":"file:///x.titan"}}}),
        ...
        assert!(text.contains("titan-lsp"));
