...
pub enum Command {
    ...
    Exec {
        #[arg(default_value = "target/program.tbc")]
        input: String,
        #[arg(long)]
        sandbox: bool,
    },
...
fn main() {
    match Cli::parse().command {
        ...
        Command::Add {
            ...
            requirement,
            project,
        } => cmd_add(&project, &package, &requirement),
        Command::Fetch {
            project,
            registry,
            offline,
        } => cmd_fetch(&project, &registry, offline),
        Command::Update { project, registry } => cmd_fetch(&project, &registry, false),
        Command::Keygen { path } => {
            titan_pkg::generate_signing_key(Path::new(&path))
                .unwrap_or_else(|error| fatal("PACKAGE ERROR", error));
            println!("Generated signing key at {path}");
        }
        Command::Pack {
            project,
            key,
            ...
            let publication =
                titan_pkg::build_package(Path::new(&project), Path::new(&key), Path::new(&output))
                    .unwrap_or_else(|error| fatal("PACKAGE ERROR", error));
            println!(
                "Packed {} {} -> {}\nsha256={}\nsigning_key={}\nsignature={}",
                publication.name,
                publication.version,
                publication.archive.display(),
                publication.sha256,
                publication.signing_key,
        ...
        Command::Debug {
            ...
            breakpoints,
            sandbox,
        } => cmd_debug(&input, &breakpoints, sandbox),
        Command::Exec { input, sandbox } => cmd_exec(&input, sandbox),
        Command::Run {
            input,
            sandbox,
            args,
        } => cmd_run(&input, sandbox, args),
...
fn cmd_add(project: &str, package: &str, requirement: &str) {
    let path = Path::new(project);
    let root = titan_pkg::find_project_root(path).unwrap_or_else(|| path.to_path_buf());
    titan_pkg::add_remote_dependency(&root, package, requirement)
        .unwrap_or_else(|error| fatal("PACKAGE ERROR", error));
    println!("Added {package} {requirement}");
}
fn cmd_fetch(project: &str, registry: &str, offline: bool) {
    let path = Path::new(project);
    let root = titan_pkg::find_project_root(path).unwrap_or_else(|| path.to_path_buf());
    let lock = titan_pkg::sync_remote_dependencies(&root, registry, offline)
        .unwrap_or_else(|error| fatal("PACKAGE ERROR", error));
    println!(
        "Synchronized {} remote packages{}",
        lock.packages.len(),
        if offline { " (offline)" } else { "" }
    );
}

fn cmd_publish(project: &str, key: &str, registry: &str) {
    let token = std::env::var("TITAN_REGISTRY_TOKEN")
        .unwrap_or_else(|_| fatal_message("PACKAGE ERROR", "TITAN_REGISTRY_TOKEN is not set"));
    let archive = std::env::temp_dir().join(format!("titan-publish-{}.tpkg", std::process::id()));
    let _ = std::fs::remove_file(&archive);
    let publication = titan_pkg::build_package(Path::new(project), Path::new(key), &archive)
        .unwrap_or_else(|error| fatal("PACKAGE ERROR", error));
    let publisher = titan_pkg::Publisher::new(registry, &token)
        .unwrap_or_else(|error| fatal("PACKAGE ERROR", error));
    publisher
        .publish(&publication)
        .unwrap_or_else(|error| fatal("PACKAGE ERROR", error));
    let _ = std::fs::remove_file(archive);
    println!("Published {} {}", publication.name, publication.version);
}

fn cmd_check(input: &str) {
    match load_and_compile(input) {
        Ok((project, module)) => println!(
            "CHECK OK: {} files, {} functions",
            project.load_order.len(),
...
fn cmd_build(input: &str, output: Option<String>) {
    let (project, module) =
        load_and_compile(input).unwrap_or_else(|error| fatal_message("COMPILATION FAILED", &error));
    let target = output
        .map(PathBuf::from)
        .unwrap_or_else(|| default_artifact(&project));
    if let Some(parent) = target.parent() {
        fs::create_dir_all(parent).unwrap_or_else(|error| fatal("BUILD ERROR", error));
    }
    let artifact = titan_codegen::BytecodeArtifact::encode(&module)
        .unwrap_or_else(|error| fatal("BUILD ERROR", error));
...
fn cmd_debug(input: &str, breakpoint_specs: &[String], sandbox: bool) {
    ...
    for specification in breakpoint_specs {
        let (path, line) = specification
            .rsplit_once(':')
            .unwrap_or_else(|| fatal_message("DEBUG ERROR", "breakpoints must use path:line"));
        let line = line
            .parse::<usize>()
            .ok()
            .filter(|line| *line > 0)
            .unwrap_or_else(|| {
                fatal_message("DEBUG ERROR", "breakpoint line must be a positive integer")
            });
        ...
        let canonical = path
            .canonicalize()
            .unwrap_or_else(|error| fatal("DEBUG ERROR", error));
        breakpoints.push(Breakpoint::SourceLine {
            source_file: canonical.to_string_lossy().into_owned(),
            line,
        });
    }
    if breakpoints.is_empty() {
        breakpoints.push(Breakpoint::Instruction {
    ...
    loop {
        match controller
            .recv()
            .unwrap_or_else(|error| fatal_message("DEBUG ERROR", &error))
        {
            DebugEvent::Stopped(frame) => {
                let location = frame
                    ...
                    .map(|location| {
                        format!(
                            "{}:{}:{}",
                            frame.source_file.as_deref().unwrap_or("<source>"),
                            location.line,
                            location.column
                        )
                    })
                    .unwrap_or_else(|| format!("{}:ip{}", frame.function_name, frame.instruction));
                println!("\nstopped at {location} [depth {}]", frame.depth);
                println!(
                    "function: {} (id {})",
                    frame.function_name, frame.function_id
                );
                println!("locals:");
                for (index, value) in frame.locals.iter().enumerate() {
                    println!("  [{index}] = {}", titan_vm::val_to_string(value));
                }
                println!("stack:");
                ...
                loop {
                    print!("debug [c=continue s=step n=next o=out p=print q=quit]> ");
                    std::io::stdout()
                        .flush()
                        .unwrap_or_else(|error| fatal("DEBUG ERROR", error));
                    ...
                    let command = match command.trim() {
                        ...
                        "s" | "step" => Some(DebugCommand::StepIn),
                        "n" | "next" => Some(DebugCommand::StepOver),
                        "o" | "out" => Some(DebugCommand::StepOut),
                        "q" | "quit" => Some(DebugCommand::Terminate),
                        "p" | "print" => {
                            println!("locals={:?}\nstack={:?}", frame.locals, frame.stack);
                            None
                        }
                        _ => {
            ...
            DebugEvent::Terminated { error } => {
                if let Some(error) = error {
                    println!("debuggee terminated: {error}");
                } else {
                    println!("debuggee exited successfully");
                }
                break;
            }
        }
    ...
    if worker.join().is_err() {
        fatal_message("DEBUG ERROR", "debuggee worker panicked");
    }
}

fn cmd_exec(input: &str, sandbox: bool) {
    let bytes = fs::read(input).unwrap_or_else(|error| fatal("ARTIFACT READ ERROR", error));
    let module = titan_codegen::BytecodeArtifact::decode(&bytes)
        .unwrap_or_else(|error| fatal("INVALID ARTIFACT", error));
    match run_module(module, sandbox) {
        Ok(Some(value)) if !matches!(value, titan_vm::Value::Nil) => {
            println!("=> {}", titan_vm::val_to_string(&value))
        }
        Ok(_) => {}
...
fn cmd_wasm(input: &str, output: Option<String>) {
    ...
    let source_map_filename = standard_map_target
        ...
        .and_then(|name| name.to_str())
        .unwrap_or_else(|| {
            fatal_message("WASM SOURCE MAP ERROR", "map filename is not valid UTF-8")
        });
...
fn cmd_test(input: &str, sandbox: bool) {
    ...
    let files = discover_titan_files(&tests_root)
        .unwrap_or_else(|error| fatal("TEST DISCOVERY ERROR", error));
    if files.is_empty() {
        fatal_message(
            "TEST ERROR",
            &format!("no .titan tests found under {}", tests_root.display()),
        );
    }
    ...
    for file in &files {
        ...
        match load_and_compile_path(file)
            .and_then(|(_, module)| run_module(module, sandbox).map_err(|error| error.to_string()))
        {
            Ok(_) => {
                passed += 1;
                println!("ok");
...
fn cmd_repl() {
    ...
    loop {
        ...
        match line {
            ":q" | ":quit" => break,
            ":h" | ":help" => {
                println!(":q quit  :h help  :v version");
                continue;
            }
            ":v" => {
                cmd_version();
        ...
        match compile_source(&source)
            .and_then(|module| run_module(module, false).map_err(|error| error.to_string()))
        {
            Ok(Some(value)) if !matches!(value, titan_vm::Value::Nil) => {
                println!("=> {}", titan_vm::val_to_string(&value))
            }
...
fn load_and_compile(
    input: &str,
) -> Result<(titan_pkg::SourceProject, titan_codegen::CompiledModule), String> {
    let entry = titan_pkg::default_entry(input);
    load_and_compile_path(&entry)
}

fn load_and_compile_path(
    entry: &Path,
) -> Result<(titan_pkg::SourceProject, titan_codegen::CompiledModule), String> {
    let project = titan_pkg::SourceProject::load(entry).map_err(|error| error.to_string())?;
    if project.manifest.is_some() {
        titan_pkg::Lockfile::from_dependencies(&project.dependencies)
            .and_then(|lock| lock.write(&project.root.join("Titan.lock")))
    ...
    fn visit(path: &Path, output: &mut Vec<PathBuf>) -> std::io::Result<()> {
        ...
        for entry in entries {
            ...
            if entry.file_type()?.is_dir() {
                visit(&path, output)?;
            } else if path.extension().and_then(|value| value.to_str()) == Some("titan") {
                output.push(path);
            }
        }
        Ok(())
...
fn fatal(label: &str, error: impl std::fmt::Display) -> ! {
