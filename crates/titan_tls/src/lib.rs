...
pub fn connect_with_timeout(
    ...
    let name = ServerName::try_from(server_name.to_owned())
        .map_err(|_| TlsError::ServerName(server_name.into()))?;
...
mod tests {
    use super::*;
    #[test]
    fn builds_webpki_client_config_and_rejects_bad_names() {
        let config = client_config();
        assert!(connect("127.0.0.1:1", "not a dns name!", config).is_err());
    }
    #[test]
    fn rejects_empty_server_credentials() {
        let root = std::env::temp_dir().join(format!("titan-tls-{}", std::process::id()));
    ...
    #[test]
    fn performs_verified_local_tls_handshake() {
        let certified = rcgen::generate_simple_self_signed(vec!["localhost".into()]).unwrap();
        let cert_pem = certified.cert.pem();
        let key_pem = certified.key_pair.serialize_pem();
        let root = std::env::temp_dir().join(format!("titan-tls-handshake-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&root);
        let cert = root.join("cert.pem");
        let key = root.join("key.pem");
        ...
        let server = std::thread::spawn(move || {
            ...
            let mut stream = accept(socket, server_config).unwrap();
            let mut data = [0; 4];
            stream.read_exact(&mut data).unwrap();
            assert_eq!(&data, b"ping");
            stream.write_all(b"pong").unwrap();
        });
        let config = client_config_with_ca(cert_pem.as_bytes()).unwrap();
        let mut client = connect(&address.to_string(), "localhost", config).unwrap();
