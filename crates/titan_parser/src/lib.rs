...
impl Parser {
    ...
    fn parse_item(&mut self) -> Result<Item> {
        ...
        match self.peek_kind() {
            Some(TokenKind::Fn) | Some(TokenKind::Extern) => {
            ...
            _ => Err(self.expected(
                "a declaration (fn, struct, enum, trait, impl, mod, import, const or type)",
            )),
        }
    }
    ...
    fn parse_impl(&mut self) -> Result<ImplBlock> {
        ...
        let (trait_name, target_type) = if self.eat(TokenKind::For) {
            let trait_name = match first {
                ...
                _ => return Err(self.message("trait name must be a named type")),
            };
    ...
    fn parse_destructure_part(&mut self) -> Result<TuplePart> {
        ...
        if let (Some(TokenKind::Ident(_)), Some(TokenKind::LBrace)) = (
    ...
    fn build_spaceship(&mut self, left: Expr, right: Expr, span: Span) -> Expr {
        ...
        let load_a = || Expr::Ident {
            name: ta.clone(),
            span,
        };
        let load_b = || Expr::Ident {
            name: tb.clone(),
            span,
        };
        let cmp = Expr::If {
            ...
            then_branch: Block {
                stmts: Vec::new(),
                final_expr: Some(Box::new(Expr::Unary {
                    op: UnaryOp::Neg,
                    expr: Box::new(Expr::Int { value: 1, span }),
                    span,
                })),
                span,
            },
    ...
    fn parse_unary(&mut self) -> Result<Expr> {
        ...
        if let Some(op) = op {
            self.advance();
            let op = if op == UnaryOp::Ref && self.eat(TokenKind::Mut) {
                UnaryOp::RefMut
            } else {
                op
            };
    ...
    fn parse_primary(&mut self) -> Result<Expr> {
        ...
        match self.peek_kind().cloned() {
            Some(TokenKind::IntLit(value)) => {
                self.advance();
                Ok(Expr::Int {
                    value: parse_int(&value, span)?,
                    span,
                })
            }
            Some(TokenKind::FloatLit(value)) => {
                self.advance();
                Ok(Expr::Float {
                    value: parse_float(&value, span)?,
                    span,
                })
            }
            Some(TokenKind::StringLit(value)) => {
                ...
                if contains_interpolation(&value) {
                    Ok(Expr::StringTemplate { value, span })
                } else {
            ...
            Some(TokenKind::LBracket) => {
                ...
                if !had_spread {
                    let elements = parts
                        ...
                        .map(|p| match p {
                            ArrayPart::Item(e) => e,
                            ArrayPart::Spread(_) => unreachable!(),
                        })
                        .collect();
            ...
            Some(TokenKind::LBrace) => {
                self.advance();
                Ok(Expr::Block(Box::new(self.parse_block_after_open(span)?)))
            }
            ...
            Some(TokenKind::Return) => {
                self.advance();
                let value =
                    if self.at_any(&[TokenKind::RBrace, TokenKind::Semicolon, TokenKind::Eof]) {
                        None
                    } else {
                        Some(Box::new(self.parse_expr()?))
                    };
    ...
    fn parse_if(&mut self) -> Result<Expr> {
        ...
        let else_branch = if self.eat(TokenKind::Else) {
            if self.at(TokenKind::If) {
                ...
                Some(Block {
                    stmts: Vec::new(),
                    final_expr: Some(Box::new(nested)),
                    span,
                })
            } else {
                self.expect(TokenKind::LBrace)?;
                Some(self.parse_block_after_open(span)?)
            }
        } else {
            None
        };
    ...
    fn parse_loop(&mut self) -> Result<Expr> {
        ...
        self.expect(TokenKind::LBrace)?;
        Ok(Expr::Loop {
            body: self.parse_block_after_open(span)?,
            span,
        })
    }
    ...
    fn parse_match(&mut self) -> Result<Expr> {
        ...
        while !self.at(TokenKind::RBrace) {
            ...
            let body = if self.eat(TokenKind::LBrace) {
                self.parse_block_after_open(arm_span)?
            } else {
                Block {
                    stmts: Vec::new(),
                    final_expr: Some(Box::new(self.parse_expr()?)),
                    span: arm_span,
                }
            };
            arms.push(MatchArm {
    ...
    fn parse_pattern(&mut self) -> Result<Pattern> {
        ...
        let mut pattern = match self.peek_kind().cloned() {
            ...
            Some(TokenKind::StringLit(value)) => {
                self.advance();
                Pattern::Literal {
                    value: Box::new(Expr::String { value, span }),
                    span,
                }
            }
            Some(TokenKind::True) => {
        ...
        while self.eat(TokenKind::Pipe) {
            pattern = Pattern::Or {
                ...
                right: Box::new(self.parse_pattern()?),
                span,
            };
        }
        Ok(pattern)
    ...
    fn at(&self, kind: TokenKind) -> bool {
        self.peek_kind().is_some_and(|k| same_variant(k, &kind))
    }
    fn at_any(&self, kinds: &[TokenKind]) -> bool {
        kinds.iter().any(|k| self.at(k.clone()))
    }
    ...
    fn expected(&self, expected: &str) -> ParseError {
        ...
        ParseError::Expected {
            ...
            line: token.map_or(0, |t| t.span.line),
            column: token.map_or(0, |t| t.span.column),
        }
    }
    fn message(&self, message: &str) -> ParseError {
        let s = self.span();
        ParseError::Message {
...
fn contains_interpolation(value: &str) -> bool {
    ...
    while let Some(open) = remainder.find('{') {
        ...
        if valid_interpolation(after_open[..close].trim()) {
            return true;
        }
...
fn valid_interpolation(value: &str) -> bool {
    ...
    let Some((function, arguments)) = value.split_once('(') else {
        return false;
    };
    if !value.ends_with(')') || !valid_path(function.trim()) {
        return false;
    }
    let arguments = &arguments[..arguments.len().saturating_sub(1)];
    arguments.trim().is_empty()
        || arguments.split(',').map(str::trim).all(|argument| {
            !argument.is_empty() && (valid_path(argument) || argument.parse::<i64>().is_ok())
        })
}
...
fn build_spread_array(parts: Vec<ArrayPart>, span: Span) -> Expr {
    ...
    let concat_name = |sp: Span| Expr::Ident {
...
fn build_map_literal(entries: Vec<(String, Expr)>, span: Span) -> Expr {
    ...
    entries
        .into_iter()
        .fold(empty, |acc, (key, value)| Expr::Call {
            callee: Box::new(Expr::Ident {
...
mod tests {
    ...
    fn distinguishes_json_braces_from_interpolation() {
        ...
        let expression = function
            .body
            .as_ref()
            .and_then(|body| body.final_expr.as_deref())
            .unwrap();
    ...
    fn parses_extern_functions() {
        let program =
            parse("extern \"C\" fn puts(s: &str) -> int; extern fn getpid() -> int;").unwrap();
