...
impl BytecodeArtifact {
    pub fn encode(module: &CompiledModule) -> Result<Vec<u8>, ArtifactError> {
        ...
        if output.len() > MAX_ARTIFACT_BYTES {
            return Err(ArtifactError::TooLarge(MAX_ARTIFACT_BYTES));
        }
        Ok(output)
    }
    ...
    pub fn decode(bytes: &[u8]) -> Result<CompiledModule, ArtifactError> {
        ...
        let payload = bytes
            .strip_prefix(MAGIC)
            .ok_or(ArtifactError::InvalidHeader)?;
        let envelope: Envelope = serde_json::from_slice(payload)?;
        if envelope.format_version != FORMAT_VERSION {
            return Err(ArtifactError::UnsupportedVersion(envelope.format_version));
        }
        let module_bytes = serde_json::to_vec(&envelope.module)?;
        if titan_stdlib::checksum::crc32(&module_bytes) != envelope.checksum_crc32 {
...
fn validate(module: &CompiledModule) -> Result<(), ArtifactError> {
    if module.functions.is_empty() {
        return invalid("module has no functions");
    }
    if module.functions.len() > MAX_FUNCTIONS {
        return invalid("function count exceeds safety limit");
    }
    if module.entry >= module.functions.len() {
        return invalid("entry function is out of bounds");
    }
    if module.string_table.len() > MAX_STRINGS {
        return invalid("string table exceeds safety limit");
    }
    ...
    if instruction_count > MAX_INSTRUCTIONS {
        return invalid("instruction count exceeds safety limit");
    }

    for (function_index, function) in module.functions.iter().enumerate() {
        if function.name.len() > 4096 {
            return invalid(&format!("function {function_index} name is too long"));
        }
        if function
            .source_file
            .as_ref()
            .is_some_and(|source| source.len() > 32_768)
        {
            return invalid(&format!(
                "function '{}' source path is too long",
                function.name
            ));
        }
        if !function.debug_locations.is_empty()
            && function.debug_locations.len() != function.code.len()
        {
            return invalid(&format!(
                "function '{}' has a malformed source map",
                function.name
            ));
        }
        if !function.param_types.is_empty() && function.param_types.len() != function.arity {
            return invalid(&format!(
                "function '{}' has malformed parameter type metadata",
                function.name
            ));
        }
        if function.captures + function.arity > function.locals {
            return invalid(&format!(
                "function '{}' has fewer locals than captures and arguments",
                function.name
            ));
        }
        if function.max_stack == 0 || function.max_stack > 1_000_000 {
            return invalid(&format!(
                "function '{}' has invalid max stack",
                function.name
            ));
        }
        for (instruction_index, instruction) in function.code.iter().enumerate() {
            let location = || {
                format!(
                    "function '{}' instruction {instruction_index}",
                    function.name
                )
            };
            match instruction {
                ...
                Op::Jump(target) | Op::JumpIfFalse(target) if *target > function.code.len() => {
                    return invalid(&format!("{} has invalid jump target {target}", location()))
                }
                Op::Call {
                    function: target,
                    argc,
                ...
                Op::CallNative { name, argc } => {
                    let signature = titan_stdlib::native::lookup(name).ok_or_else(|| {
                        ArtifactError::Invalid(format!(
                            "{} calls unknown native '{name}'",
                            location()
                        ))
                    })?;
                    if *argc != signature.params.len() {
                        return invalid(&format!(
                            "{} calls native '{name}' with wrong arity",
                            location()
                        ));
                    }
                }
                Op::MakeClosure {
                    ...
                    let target_function = module.functions.get(*target).ok_or_else(|| {
                        ArtifactError::Invalid(format!(
                            "{} references missing closure {target}",
                            location()
                        ))
                    })?;
                    if captures.len() != target_function.captures {
                        return invalid(&format!(
                            "{} supplies wrong capture count for '{}'",
                            location(),
                            target_function.name
                        ));
                    }
                    if captures.iter().any(|capture| *capture >= function.locals) {
                        return invalid(&format!("{} captures a missing local", location()));
                    }
                }
                Op::NewArray(count)
                | Op::NewTuple(count)
                ...
                | Op::CallValue(count)
                    if *count > 1_000_000 =>
                {
                    return invalid(&format!("{} has an excessive operand count", location()))
                }
                Op::NewStruct { name, fields } if name.len() > 4096 || fields.len() > 100_000 => {
                    return invalid(&format!("{} has invalid struct metadata", location()))
                }
                Op::PushFloat(value) if !value.is_finite() => {
                    return invalid(&format!("{} contains a non-finite float", location()))
                }
                _ => {}
            }
        }
...
mod tests {
    ...
    fn module() -> CompiledModule {
        CompiledModule {
            functions: vec![BytecodeFunc {
                ...
                locals: 0,
                max_stack: 8,
                code: vec![Op::PushInt(42), Op::Ret],
                debug_locations: vec![None, None],
            }],
            entry: 0,
    ...
    fn round_trips_valid_artifacts() {
        let module = module();
        let encoded = BytecodeArtifact::encode(&module).unwrap();
        assert!(encoded.starts_with(MAGIC));
        assert_eq!(BytecodeArtifact::decode(&encoded).unwrap(), module);
    }
    ...
    fn rejects_corruption_and_invalid_references() {
        ...
        let mut invalid_module = module();
        invalid_module.functions[0]
            .code
